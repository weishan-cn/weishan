...
function main() {
  ...
  assert.equal(api.READ_ONLY_QUOTE_RUN_TIMELINE_VERSION, "2.4.1");
  const timeline = api.buildReadOnlyQuoteRunTimeline({ runId:"deterministic-v2.4.1-read-only-sandbox-run", status:"completed" });
  assert.equal(timeline.appVersion, "2.4.1");
  ...
  assert.equal(audit.appVersion, "2.4.1");
