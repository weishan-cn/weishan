...
function main() {
  ...
  assert.equal(api.READ_ONLY_QUOTE_RUN_TIMELINE_VERSION, "2.1.99");
  const timeline = api.buildReadOnlyQuoteRunTimeline({ runId:"deterministic-v2.1.99-read-only-sandbox-run", status:"completed" });
  assert.equal(timeline.appVersion, "2.1.99");
  ...
  assert.equal(audit.appVersion, "2.1.99");
