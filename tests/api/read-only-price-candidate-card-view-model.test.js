...
function main() {
  ...
  const publicBetaQaFreezeGateSummary = { status:"ready", userFacingSummary:{ title:"Public Beta QA Freeze Gate", resultLabel:"Public Beta QA Freeze Gate 已准备", redacted:true }, redacted:true };
  const manualTrialSummaryBoardSummary = { status:"ready", userFacingSummary:{ title:"Manual Trial Summary Board", resultLabel:"Manual Trial Summary Board 已准备", redacted:true }, redacted:true };
  const offlineReadinessReviewPanelSummary = { status:"manual_review_required", userFacingSummary:{ title:"Offline Readiness Review Panel", resultLabel:"Offline Readiness Review Panel 进入人工复核", redacted:true }, redacted:true };
  const publicBetaFreezeReviewViewModelSummary = { status:"ready", title:"Public Beta Freeze Review View Model", userFacingSummary:{ title:"Public Beta Freeze Review View Model", resultLabel:"Public Beta QA Freeze Gate / Manual Trial Summary Board / Offline Readiness Review Panel 已准备", redacted:true }, safeToProceedWithManualFreezeReview:true, redacted:true };
  ...
  assert.equal(renderedCardHtml.includes("Public Beta QA Freeze Gate"), true);
  assert.equal(renderedCardHtml.includes("Manual Trial Summary Board"), true);
  assert.equal(renderedCardHtml.includes("Offline Readiness Review Panel"), true);
  assert.equal(renderedCardHtml.includes("Allowed Next Actions"), true);
  assert.equal(renderedCardHtml.includes("Blocked Next Actions"), true);
  ...
  assert.equal(renderedCardHtml.includes("当前冻结的是只读 QA 范围，不执行真实 freeze"), true);
  ...
  assert.equal(renderedCardHtml.includes("人工复核后再决定下一阶段"), true);
  assert.equal(card.safeToProceedWithManualFreezeReview, true);
