...
function main() {
  ...
  assert.equal(html.includes("Provider Launch Control Tower"), true);
  assert.equal(html.includes("Offline Provider Launch Control Tower"), true);
  assert.equal(html.includes("Adapter Policy Engine"), true);
  assert.equal(html.includes("Human Release Evidence Timeline"), true);
  assert.equal(html.includes("Sandbox Activation Final Review Board"), true);
  assert.equal(html.includes("Launch Control"), true);
  assert.equal(html.includes("Adapter Policy"), true);
  assert.equal(html.includes("Evidence Timeline"), true);
  assert.equal(html.includes("Final Review"), true);
  assert.equal(html.includes("离线 Launch 控制塔已准备"), true);
  assert.equal(html.includes("Adapter 策略引擎已准备"), true);
  assert.equal(html.includes("人工发布证据时间线已准备"), true);
  assert.equal(html.includes("Sandbox 激活终审板已准备"), true);
  assert.equal(html.includes("Launch Control 不保存真实决策"), true);
  assert.equal(html.includes("Adapter Policy 不修改配置、不启用 provider"), true);
  assert.equal(html.includes("Evidence Timeline 不持久化时间线"), true);
  assert.equal(html.includes("Final Review 不激活 sandbox"), true);
  assert.equal(html.includes("Human launch control review 仍需人工复核"), true);
  assert.equal(html.includes("当前只展示 provider launch control tower"), true);
