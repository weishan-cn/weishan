...
function main() {
  ...
  const publicBetaTrialEvidenceLedgerSummary = { status:"ready", userFacingSummary:{ title:"Public Beta Trial Evidence Ledger", resultLabel:"Public Beta Trial Evidence Ledger 已准备", redacted:true }, redacted:true };
  const publicBetaQaDecisionMatrixSummary = { status:"ready", userFacingSummary:{ title:"QA Decision Matrix", resultLabel:"QA Decision Matrix 已准备", redacted:true }, redacted:true };
  const offlineIssueTriageBoardSummary = { status:"ready", userFacingSummary:{ title:"Offline Issue Triage Board", resultLabel:"Offline Issue Triage Board 已准备", redacted:true }, redacted:true };
  const publicBetaQaOperationsViewModelSummary = { status:"ready", title:"Public Beta Trial Evidence Ledger", userFacingSummary:{ title:"Public Beta QA Operations View Model", resultLabel:"Public Beta Trial Evidence Ledger / QA Decision Matrix / Offline Issue Triage Board 已准备", redacted:true }, safeToProceedWithManualQaOperationsReview:true, redacted:true };
  ...
  assert.equal(renderedCardHtml.includes("Public Beta Trial Evidence Ledger"), true);
  assert.equal(renderedCardHtml.includes("QA Decision Matrix"), true);
  assert.equal(renderedCardHtml.includes("Offline Issue Triage Board"), true);
  assert.equal(renderedCardHtml.includes("Allowed Decisions"), true);
  assert.equal(renderedCardHtml.includes("Blocked Decisions"), true);
  assert.equal(renderedCardHtml.includes("Manual Review Items"), true);
  assert.equal(renderedCardHtml.includes("Feedback Review"), true);
  assert.equal(renderedCardHtml.includes("Next Manual Action"), true);
  ...
  assert.equal(renderedCardHtml.includes("Locked Capabilities"), true);
  assert.equal(renderedCardHtml.includes("Next Decision Options"), true);
  assert.equal(renderedCardHtml.includes("Continue Testing"), true);
  assert.equal(renderedCardHtml.includes("Manual Review Required"), true);
  assert.equal(renderedCardHtml.includes("当前仍为只读 Public Beta 候选"), true);
  assert.equal(renderedCardHtml.includes("不自动发布、不接 provider、不启用交易"), true);
  assert.equal(renderedCardHtml.includes("只允许继续测试、人工复核或阻断"), true);
  assert.equal(renderedCardHtml.includes("问题分流仅为离线视图，不创建真实 issue"), true);
  assert.equal(renderedCardHtml.includes("不自动发布、不启用 provider、不启用交易"), true);
  assert.equal(renderedCardHtml.includes("可继续人工试用和问题记录"), true);
  assert.equal(renderedCardHtml.includes("既有 secret scan WARN 仅作为已知警告展示"), true);
