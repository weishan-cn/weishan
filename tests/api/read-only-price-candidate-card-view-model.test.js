...
function main() {
  ...
  assert.equal(html.includes("Public Beta 只提供候选价证据，不付款、不下单、不出票"), true);
  assert.equal(html.includes("Provider-Zero Lock 不接真实 provider、不读密钥、不联网"), true);
  assert.equal(html.includes("User Trust Launch 不执行真实 launch"), true);
  assert.equal(html.includes("Safety Copy 不承诺最低价、最终价或官方背书"), true);
  assert.equal(html.includes("Human public beta review 仍需人工复核"), true);
  assert.equal(html.includes("当前只展示 Global Shopping Public Beta Review"), true);
