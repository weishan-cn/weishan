...
function main() {
  const windowRef = load(["apps/desktop/src/renderer/core/globalShoppingPriceSourceNormalizer.js", "apps/desktop/src/renderer/core/globalShoppingOfficialPriceAnchorSlot.js", "apps/desktop/src/renderer/core/globalShoppingPriceCandidateDisplayBoard.js"]);
  const api = windowRef.WeishanGlobalShoppingPriceCandidateDisplayBoard;
  assert.equal(api.GLOBAL_SHOPPING_PRICE_CANDIDATE_DISPLAY_BOARD_VERSION, "2.1.89");
  const board = api.buildGlobalShoppingPriceCandidateDisplayBoard();
  assert.equal(board.appVersion, "2.1.89");
  ...
  assert.equal(board.cards.find((item) => item.cardId === "source_trust").label, "来源与可信度");
