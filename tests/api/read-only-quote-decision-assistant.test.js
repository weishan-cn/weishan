...
function main() {
  ...
  assert.equal(api.READ_ONLY_QUOTE_DECISION_ASSISTANT_VERSION, "2.1.61");
  ...
  const model = api.buildReadOnlyQuoteDecisionAssistant({ workflowStateSummary:{ status:"evidence_ready" }, clarificationSummary:{ status:"complete" }, workflowStepList:[{ label:"生成候选证据", status:"completed" }], missingFields:[], clarificationQuestions:[], workflowUserMessage:"候选证据已生成，平台最终为准。", topCandidates:[
  ...
  assert.equal(model.appVersion, "2.1.61");
