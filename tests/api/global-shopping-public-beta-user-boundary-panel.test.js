...
function main() {
  ...
  assert.equal(api.GLOBAL_SHOPPING_PUBLIC_BETA_USER_BOUNDARY_PANEL_VERSION, "4.0.6");
  const ready = api.buildGlobalShoppingPublicBetaUserBoundaryPanel({ appVersion:"4.0.6", panelMode:"user_boundary_only" });
  ...
  assert.equal(api.buildGlobalShoppingPublicBetaUserBoundaryPanel({ appVersion:"4.0.6", panelMode:"user_boundary_only", openExternal:true }).status, "blocked");
