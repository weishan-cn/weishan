...
function main() {
  ...
  assert.equal(api.FLIGHT_WORKFLOW_RELEASE_READINESS_VIEW_MODEL_VERSION, "4.1.5");
  const dashboard = { status:"ready", releaseVersion:"4.1.5", safeForUserFacingBeta:true, readiness:{ status:"ready", matrixBlocked:false, warningCount:0, operatorReady:true, redacted:true }, cards:[], checklistRows:[{ label:"安全矩阵", passed:true, value:"通过", redacted:true }], forbiddenCapabilities:["付款", "下单", "出票", "证件银行卡上传"], userFacingSummary:{ resultLabel:"可以进入只读 Beta 验收", redacted:true }, redacted:true };
