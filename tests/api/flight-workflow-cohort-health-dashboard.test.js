...
function main() {
  ...
  assert.equal(api.FLIGHT_WORKFLOW_COHORT_HEALTH_DASHBOARD_VERSION, "2.1.90");
  ...
  assert.equal(healthy.pilotOpsStatus, "");
  assert.equal(healthy.nextCohortDecisionStatus, "");
  assert.equal(healthy.pilotOpsPrimaryRisk, null);
