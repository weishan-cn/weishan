...
async function main(){
  ...
  const preload = fs.readFileSync(path.join(ROOT, "apps/desktop/src/preload.js"), "utf8"); vm.runInContext(preload, vm.createContext({ require:(name) => name === "electron" ? electron : name === "./shared/videoProviderIpcContract" ? contract : name === "./shared/ipcTrustBoundary" ? ipcTrustBoundary : name === "../package.json" ? { version:"0.0.0-test", productName:"Weishan" } : (() => { throw new Error(name); })(), process:{ env:{} }, console }));
