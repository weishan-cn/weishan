...
async function main(){
  ...
  const preload = fs.readFileSync(path.join(ROOT, "apps/desktop/src/preload.js"), "utf8"); vm.runInContext(preload, vm.createContext({ require:(name) => name === "electron" ? electron : name === "./shared/videoProviderIpcContract" ? contract : (() => { throw new Error(name); })(), process:{ env:{} }, console }));
