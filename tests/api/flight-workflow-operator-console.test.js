...
function main() {
  ...
  assert.equal(api.FLIGHT_WORKFLOW_OPERATOR_CONSOLE_VERSION, "3.0.0");
  ...
  })).sections.find((section) => section.sectionId === "global_shopping_provider_launch_control");
  assert.ok(launchControlRows.rows.some((item) => item.label === "Launch Control"));
  assert.ok(launchControlRows.rows.some((item) => item.label === "Adapter Policy"));
  assert.ok(launchControlRows.rows.some((item) => item.label === "Evidence Timeline"));
  assert.ok(launchControlRows.rows.some((item) => item.label === "Final Review"));
