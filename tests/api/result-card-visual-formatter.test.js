...
function main(){
  ...
  assert.equal(api.RESULT_CARD_VISUAL_FORMATTER_VERSION, "2.1.38");
  assert.equal(badgeApi.RESULT_BADGE_FORMATTER_VERSION, "2.1.38");
  ...
  const model = api.buildResultCardVisualModel({
    ...
    card:{ title:"上海 → 成都 · 7 月 15 日 · 直达优先 · 低价优先", providerName:"Flight Provider Sandbox", priceDisplay:"¥1010", updatedAt:"2026-06-20T00:00:00.000Z", priceTruthLabel:"Limited Beta 只读验证价，不代表真实最低价", badges:["Limited Beta", "只读价格", "不可下单", "最终以平台页面为准"], fareBreakdown:fare }
  });
  ...
  assert.equal(model.priceSubtext, "Limited Beta 只读验证价，不代表真实最低价");
  ...
  assert.deepEqual(Array.from(model.badges), ["Limited Beta", "只读价格", "不可下单", "以平台页面为准"]);
  assert.equal(model.badgeDisplayText, "[Limited Beta] [只读价格] [不可下单] [以平台页面为准]");
