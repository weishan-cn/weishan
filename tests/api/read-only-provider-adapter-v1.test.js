...
function main() {
  assert.equal(contractApi.READ_ONLY_PROVIDER_ADAPTER_CONTRACT_VERSION, "2.1.63");
  const contract = contractApi.buildAdapterContract();
  assert.equal(contract.contractVersion, "2.1.63");
  ...
  assert.equal(gate.version, "2.1.63");
  ...
  assert.equal(flightApi.FLIGHT_READONLY_PROVIDER_ADAPTER_V1_VERSION, "2.1.63");
