...
function main() {
  assert.equal(contractApi.READ_ONLY_PROVIDER_ADAPTER_CONTRACT_VERSION, "4.1.4");
  const contract = contractApi.buildAdapterContract();
  assert.equal(contract.contractVersion, "4.1.4");
  ...
  assert.equal(gate.version, "4.1.4");
  ...
  assert.equal(flightApi.FLIGHT_READONLY_PROVIDER_ADAPTER_V1_VERSION, "4.1.4");
