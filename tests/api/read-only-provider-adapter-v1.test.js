...
function main() {
  assert.equal(contractApi.READ_ONLY_PROVIDER_ADAPTER_CONTRACT_VERSION, "4.2.7");
  const contract = contractApi.buildAdapterContract();
  assert.equal(contract.contractVersion, "4.2.7");
  ...
  assert.equal(gate.version, "4.2.7");
  ...
  assert.equal(flightApi.FLIGHT_READONLY_PROVIDER_ADAPTER_V1_VERSION, "4.2.7");
