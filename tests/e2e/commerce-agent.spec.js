...
test.describe.serial("commerce agent workbench", () => {
  ...
  test("v2.1.4 provider source label and price integrity gates stay draft-only and blocked", async () => {
    ...
    expect(contracts.source.gate).toEqual(expect.objectContaining({ version:"2.1.35", phase:"provider_result_source_label_gate", gateStatus:"closed", mode:"draft_only", realProviderSourceLabel:"disabled", realProviderResultRead:"disabled", realNetwork:"disabled", realPriceDisplay:"disabled", realAvailabilityDisplay:"disabled", realBookingUrlDisplay:"disabled", rawProviderPayloadDisplay:"forbidden" }));
    ...
    expect(contracts.price.gate).toEqual(expect.objectContaining({ version:"2.1.35", phase:"price_integrity_taxes_fees_gate", gateStatus:"closed", mode:"draft_only", realPriceDisplay:"disabled", realProviderPrice:"disabled", taxFeeVerification:"disabled_until_readonly_provider_result_available", realProviderResultRead:"disabled", realNetwork:"disabled", realBookingUrlDisplay:"disabled" }));
  ...
  test("v2.1.35 bookingUrl domain safety and manual provider review gates stay draft-only and blocked", async () => {
    await resetCommerceTasks(page);
    ...
    expect(contracts.booking.gate).toEqual(expect.objectContaining({ version:"2.1.35", phase:"booking_url_domain_safety_gate", gateStatus:"closed", mode:"draft_only", bookingUrlDisplay:"disabled", bookingUrlGeneration:"disabled", bookingUrlClick:"disabled", redirectFollow:"disabled", realProviderBookingLink:"disabled", realNetwork:"disabled" }));
  ...
  test("v2.1.4 trusted external search router keeps lowest two flight offers contract gated and candidate registry collapsed", async () => {
    ...
    expect(secureKeyStoragePlan.contract).toEqual(expect.objectContaining({
      secureKeyStoragePlanVersion:"2.1.35",
  ...
  test("v2.1.4 sidebar version stays in sync with release version", async () => {
    ...
    await expect(sidebarFoot).toContainText("weishan v2.1.35");
  ...
  test("v2.1.35 provider activation readiness gates stay blocked and visible", async () => {
    await resetCommerceTasks(page);
    await gotoRoute(page, "home");
    const latestButton = page.locator("#taskHistoryLatestBtn");
    if (await latestButton.count()) await latestButton.click();
    ...
    expect(gateContracts.activationContract).toEqual(expect.objectContaining({ version:"2.1.35", gateStatus:"blocked", mode:"readiness_only", activationGoNoGo:"no-go", realProviderConnection:"disabled", realPrice:"disabled", realBookingUrl:"disabled", orderMode:"disabled", paymentMode:"disabled" }));
    ...
    expect(gateContracts.consentContract).toEqual(expect.objectContaining({ version:"2.1.35", gateStatus:"draft-ready", mode:"no provider connection", realCredentialInput:"disabled", realCredentialSave:"disabled", realCredentialRead:"disabled", finalDecision:"no-go" }));
    ...
    expect(gateContracts.adapterContract).toEqual(expect.objectContaining({ version:"2.1.35", gateStatus:"draft-ready", mode:"offline_fixture_only", adapterExecution:"offline fixture only", realNetwork:"disabled", realEndpoint:"disabled", realProviderSandbox:"disabled", realProviderResult:"disabled", rawPayloadDisplay:"disabled", writeAction:"disabled" }));
  ...
  test("v2.1.35 offline provider compliance harness stays offline and blocked", async () => {
    await resetCommerceTasks(page);
    await gotoRoute(page, "home");
    const latestButton = page.locator("#taskHistoryLatestBtn");
    if (await latestButton.count()) await latestButton.click();
    ...
    const summaryPanel = home.locator(".commerce-simple-flight-result");
    await expect(summaryPanel).toHaveCount(1, { timeout:15000 });
    await expect(page.locator(".sidebar-foot")).toContainText("weishan v2.1.35");
    ...
    expect(gateContracts.matrixContract).toEqual(expect.objectContaining({ version:"2.1.35", dashboardStatus:"blocked", mode:"matrix_only", providerActivationState:"no-go", realNetwork:"disabled", realPrice:"disabled", realBookingUrl:"disabled" }));
    ...
    expect(gateContracts.guardContract).toEqual(expect.objectContaining({ version:"2.1.35", guardStatus:"blocked", mode:"no_network_enforcement_draft", providerNetwork:"disabled", adapterExecution:"disabled" }));
    ...
    expect(gateContracts.harnessContract).toEqual(expect.objectContaining({ version:"2.1.35", harnessStatus:"offline_only", mode:"fixture_validation_draft", realProviderFixture:"disabled", realProviderResult:"disabled", realNetwork:"disabled", fakeMockDemoAiPriceDisplay:"disabled", bookingUrlDisplay:"disabled", rawProviderPayloadDisplay:"disabled" }));
    ...
    expect(gateContracts.decisionContract).toEqual(expect.objectContaining({ version:"2.1.35", engineStatus:"blocked", mode:"offline_decision_only", sideEffects:"none", realProviderConnection:"disabled", realNetwork:"disabled", realCredentialRead:"disabled", realPriceDisplay:"disabled", realBookingUrl:"disabled", providerActivationDecision:"no-go" }));
    ...
    expect(gateContracts.runnerContract).toEqual(expect.objectContaining({ version:"2.1.35", runnerStatus:"offline_only", mode:"deterministic_fixture_runner", realProviderFixture:"disabled", realProviderResult:"disabled", realNetwork:"disabled", realPrice:"disabled", fakeMockDemoAiPriceDisplay:"disabled", bookingUrlDisplay:"disabled", rawProviderPayloadDisplay:"disabled" }));
    ...
    expect(gateContracts.sentinelContract).toEqual(expect.objectContaining({ version:"2.1.35", sentinelStatus:"blocked", mode:"static_no_network_audit", globalMonkeyPatch:"disabled", providerNetworkCall:"disabled", fetchAttempt:"blocked", xhrAttempt:"blocked", websocketAttempt:"blocked" }));
    ...
    expect(gateContracts.evidenceContract).toEqual(expect.objectContaining({ version:"2.1.35", reportStatus:"blocked", mode:"offline_evidence_only", providerActivationState:"no-go", realProviderApproval:"none", credentialConsentApproval:"none", realSecureStorage:"disabled", realEndpointConnection:"disabled", realSandbox:"disabled", realProviderResult:"disabled", realPrice:"disabled", realBookingUrl:"disabled" }));
  ...
  test("v2.1.35 local safety evidence console and no-secret persistence guard stay local only", async () => {
    ...
    await expect(page.locator(".sidebar-foot")).toContainText("weishan v2.1.35", { timeout:15000 });
    ...
    for (const text of [
      ...
      "/tmp/weishan-v2.1.35-ui-acceptance/01_app_launched.png",
      "/tmp/weishan-v2.1.35-ui-acceptance/10_compound_request.png",
    ...
    expect(contracts.consoleContract).toEqual(expect.objectContaining({ version:"2.1.35", status:"local evidence only", mode:"offline safety summary", providerActivationState:"no-go", releaseEvidenceState:"local only" }));
    ...
    expect(contracts.manualContract).toEqual(expect.objectContaining({ version:"2.1.35", status:"manual assist only", mode:"no automation guarantee", automatedPassFabrication:"forbidden", screenshotEvidence:"required" }));
    ...
    expect(contracts.guardContract).toEqual(expect.objectContaining({ version:"2.1.35", status:"local static scan only", mode:"no real secret access", realApiKeyRead:"disabled", keychainAccess:"disabled", safeStorageAccess:"disabled" }));
    ...
    expect(contracts.authContract).toEqual(expect.objectContaining({ version:"2.1.35", status:"local auth evidence only", mode:"no cloud auth", localRegister:"enabled", localLogin:"enabled", localRecoveryNotice:"enabled", passwordVerifier:"enabled", legacyPlainPasswordMigration:"compatible" }));
  ...
  test("v2.1.35 global procurement user-facing result cards stay offline and category-specific", async () => {
    await resetCommerceTasks(page);
    await gotoRoute(page, "commerce");
    ...
    expect(contracts.checklist).toEqual(expect.objectContaining({ checklistVersion:"2.1.35", status:"draft only", mode:"local planning only", realProvider:"disabled", realNetwork:"disabled", redacted:true }));
    expect(contracts.guidance).toEqual(expect.objectContaining({ guidanceVersion:"2.1.35", status:"safe guidance only", mode:"no transaction", realProvider:"disabled", realNetwork:"disabled", payment:"disabled", order:"disabled", redacted:true }));
    expect(contracts.policy).toEqual(expect.objectContaining({ policyVersion:"2.1.35", status:"manual external search only", autoClick:"disabled", bookingUrl:"disabled", realProvider:"disabled", realNetwork:"disabled", redacted:true, allowExternalSearch:true }));
    expect(contracts.blockedPolicy.allowExternalSearch).toBe(false);
    expect(contracts.flightDetail).toEqual(expect.objectContaining({ detailQualityVersion:"2.1.35", title:"机票搜索计划", emptyResultLine:"暂无真实价格结果", currentStatusLine:"当前为离线采购规划 / 只整理条件 / 不接真实平台。", redacted:true }));
    ...
    expect(contracts.flightCard).toEqual(expect.objectContaining({ cardVersion:"2.1.35", title:"机票搜索计划", categoryLabel:"机票", currentStatusLine:"仅整理搜索条件，暂无真实价格", redacted:true }));
    ...
    expect(contracts.resultCardRules).toEqual(expect.objectContaining({ rulesVersion:"2.1.35", status:"user-facing summary only", realProvider:"disabled", realNetwork:"disabled", realPrice:"disabled", bookingUrl:"disabled", payment:"disabled", order:"disabled", identityUpload:"disabled", redacted:true }));
  ...
  test("v2.1.35 global procurement decision workspace stays offline and visible", async () => {
    await resetCommerceTasks(page);
    await gotoRoute(page, "home");
  ...
  test("v2.1.35 ticket activity result stays isolated and keeps readonly provider result schema gate", async () => {
    await resetCommerceTasks(page);
    await gotoRoute(page, "home");
  ...
  test("v2.1.35 commerce normal category smoke stays clean result surface only @commerce-smoke", async () => {
    await resetCommerceTasks(page);
    await gotoRoute(page, "home");

    const cases = [
      {
        input:runId + "-SMOKE-FLIGHT 7月15日上海到成都最便宜的直达机票",
        expected:["机票搜索结果", "出发地：上海", "目的地：成都", "日期：7 月 15 日", "排序：低价优先", "推荐结果", "Limited Beta", "暂无生产真实最低价", "不代表真实最低价", "去平台确认"]
      },
      {
    ...
    for (const item of cases) {
      ...
      if (item.input.includes("SMOKE-FLIGHT")) {
        ...
        await expect(summary.locator(".commerce-top-result-card").first()).toContainText("¥1010");
        await expect(summary.locator(".commerce-top-result-card").first()).toContainText("价格拆分");
        await expect(summary.locator(".commerce-top-result-card").first()).toContainText("票面价");
        await expect(summary.locator(".commerce-top-result-card").first()).toContainText("燃油附加费");
        await expect(summary.locator(".commerce-top-result-card").first()).toContainText("机场建设费 / 民航发展基金");
        await expect(summary.locator(".commerce-top-result-card").first()).toContainText("税费");
        await expect(summary.locator(".commerce-top-result-card").first()).toContainText("其它附加费");
        await expect(summary.locator(".commerce-top-result-card").first()).toContainText("最终应付总价：¥1010");
        await expect(summary.locator(".commerce-top-result-card").first()).toContainText("是否可声称最便宜：否");
        await expect(summary.locator(".commerce-top-result-card").first()).toContainText("not_ranked_as_real_cheapest");
        await expect(summary.locator(".commerce-top-result-card").first()).toContainText("最终以平台页面为准");
        ...
        await expect(handoffPanel).toContainText("票面价：¥860");
        await expect(handoffPanel).toContainText("燃油附加费：未单独提供");
        await expect(handoffPanel).toContainText("最终应付总价：¥1010");
      ...
      if (item.input.includes("SMOKE-FLIGHT")) {
        ...
        for (const text of ["Clean Result Surface V2", "clean result surface v2: active", "debugPanelsHiddenByDefault: true", "backendPanelDefaultExpandedCount: 0", "bookingUrlDisplayedCount: 0", "paymentButtonDisplayedCount: 0", "orderButtonDisplayedCount: 0", "identityUploadDisplayedCount: 0", "CLEAN_RESULT_SURFACE_V2_DRAFT", "redacted: true"]) await expect(cleanV2Body).toContainText(text);
  ...
  test("v2.1.35 vague procurement requests ask clarification without fake results @commerce-smoke", async () => {
    await resetCommerceTasks(page);
    await gotoRoute(page, "home");
  ...
  test("v2.1.35 secure API key storage console stays metadata-only and offline @commerce-smoke", async () => {
    await resetCommerceTasks(page);
    await gotoRoute(page, "home");
  ...
  test("v2.1.35 secure API key storage test actions never reveal plaintext @commerce-smoke", async () => {
    await resetCommerceTasks(page);
    await gotoRoute(page, "home");
  ...
  test("v2.1.35 restricted request smoke strips procurement actions @commerce-smoke", async () => {
    await resetCommerceTasks(page);
    await gotoRoute(page, "home");
  ...
  test("v2.1.35 latest command priority keeps newest ticket task active and topmost @latest-command-priority", async () => {
    await resetCommerceTasks(page);
    await gotoRoute(page, "home");
  ...
  test("v2.1.35 history detail viewing does not intercept newly executed task @latest-command-priority", async () => {
    await resetCommerceTasks(page);
    await gotoRoute(page, "home");
  ...
  test("v2.1.35 latest flight request is not overridden by older ticket task @latest-command-priority", async () => {
    await resetCommerceTasks(page);
    await gotoRoute(page, "home");
  ...
  test("v2.1.35 clear finished preserves latest draft and executes newest input @latest-command-priority", async () => {
    await resetCommerceTasks(page);
    await gotoRoute(page, "home");
