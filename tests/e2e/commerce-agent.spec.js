...
test.describe.serial("commerce agent workbench", () => {
  ...
  test("v2.1.4 provider source label and price integrity gates stay draft-only and blocked", async () => {
    ...
    expect(contracts.source.gate).toEqual(expect.objectContaining({ version:"2.1.26", phase:"provider_result_source_label_gate", gateStatus:"closed", mode:"draft_only", realProviderSourceLabel:"disabled", realProviderResultRead:"disabled", realNetwork:"disabled", realPriceDisplay:"disabled", realAvailabilityDisplay:"disabled", realBookingUrlDisplay:"disabled", rawProviderPayloadDisplay:"forbidden" }));
    ...
    expect(contracts.price.gate).toEqual(expect.objectContaining({ version:"2.1.26", phase:"price_integrity_taxes_fees_gate", gateStatus:"closed", mode:"draft_only", realPriceDisplay:"disabled", realProviderPrice:"disabled", taxFeeVerification:"disabled_until_readonly_provider_result_available", realProviderResultRead:"disabled", realNetwork:"disabled", realBookingUrlDisplay:"disabled" }));
  ...
  test("v2.1.26 bookingUrl domain safety and manual provider review gates stay draft-only and blocked", async () => {
    await resetCommerceTasks(page);
    ...
    expect(contracts.booking.gate).toEqual(expect.objectContaining({ version:"2.1.26", phase:"booking_url_domain_safety_gate", gateStatus:"closed", mode:"draft_only", bookingUrlDisplay:"disabled", bookingUrlGeneration:"disabled", bookingUrlClick:"disabled", redirectFollow:"disabled", realProviderBookingLink:"disabled", realNetwork:"disabled" }));
  ...
  test("v2.1.4 trusted external search router keeps lowest two flight offers contract gated and candidate registry collapsed", async () => {
    ...
    expect(secureKeyStoragePlan.contract).toEqual(expect.objectContaining({
      secureKeyStoragePlanVersion:"2.1.26",
  ...
  test("v2.1.4 sidebar version stays in sync with release version", async () => {
    ...
    await expect(sidebarFoot).toContainText("weishan v2.1.26");
  ...
  test("v2.1.26 provider activation readiness gates stay blocked and visible", async () => {
    await resetCommerceTasks(page);
    await gotoRoute(page, "home");
    const latestButton = page.locator("#taskHistoryLatestBtn");
    if (await latestButton.count()) await latestButton.click();
    ...
    expect(gateContracts.activationContract).toEqual(expect.objectContaining({ version:"2.1.26", gateStatus:"blocked", mode:"readiness_only", activationGoNoGo:"no-go", realProviderConnection:"disabled", realPrice:"disabled", realBookingUrl:"disabled", orderMode:"disabled", paymentMode:"disabled" }));
    ...
    expect(gateContracts.consentContract).toEqual(expect.objectContaining({ version:"2.1.26", gateStatus:"draft-ready", mode:"no provider connection", realCredentialInput:"disabled", realCredentialSave:"disabled", realCredentialRead:"disabled", finalDecision:"no-go" }));
  ...
  test("v2.1.26 offline provider compliance harness stays offline and blocked", async () => {
    await resetCommerceTasks(page);
    await gotoRoute(page, "home");
    const latestButton = page.locator("#taskHistoryLatestBtn");
    if (await latestButton.count()) await latestButton.click();
    ...
    const summaryPanel = home.locator(".commerce-simple-flight-result");
    await expect(summaryPanel).toHaveCount(1, { timeout:15000 });
    await expect(page.locator(".sidebar-foot")).toContainText("weishan v2.1.26");
    ...
    expect(gateContracts.matrixContract).toEqual(expect.objectContaining({ version:"2.1.26", dashboardStatus:"blocked", mode:"matrix_only", providerActivationState:"no-go", realNetwork:"disabled", realPrice:"disabled", realBookingUrl:"disabled" }));
    ...
    expect(gateContracts.guardContract).toEqual(expect.objectContaining({ version:"2.1.26", guardStatus:"blocked", mode:"no_network_enforcement_draft", providerNetwork:"disabled", adapterExecution:"disabled" }));
    ...
    expect(gateContracts.decisionContract).toEqual(expect.objectContaining({ version:"2.1.26", engineStatus:"blocked", mode:"offline_decision_only", sideEffects:"none", realProviderConnection:"disabled", realNetwork:"disabled", realCredentialRead:"disabled", realPriceDisplay:"disabled", realBookingUrl:"disabled", providerActivationDecision:"no-go" }));
    ...
    expect(gateContracts.sentinelContract).toEqual(expect.objectContaining({ version:"2.1.26", sentinelStatus:"blocked", mode:"static_no_network_audit", globalMonkeyPatch:"disabled", providerNetworkCall:"disabled", fetchAttempt:"blocked", xhrAttempt:"blocked", websocketAttempt:"blocked" }));
  ...
  test("v2.1.26 local safety evidence console and no-secret persistence guard stay local only", async () => {
    ...
    await expect(page.locator(".sidebar-foot")).toContainText("weishan v2.1.26", { timeout:15000 });
    ...
    for (const text of [
      ...
      "/tmp/weishan-v2.1.26-ui-acceptance/01_app_launched.png",
      "/tmp/weishan-v2.1.26-ui-acceptance/10_compound_request.png",
    ...
    expect(contracts.consoleContract).toEqual(expect.objectContaining({ version:"2.1.26", status:"local evidence only", mode:"offline safety summary", providerActivationState:"no-go", releaseEvidenceState:"local only" }));
    ...
    expect(contracts.manualContract).toEqual(expect.objectContaining({ version:"2.1.26", status:"manual assist only", mode:"no automation guarantee", automatedPassFabrication:"forbidden", screenshotEvidence:"required" }));
    ...
    expect(contracts.guardContract).toEqual(expect.objectContaining({ version:"2.1.26", status:"local static scan only", mode:"no real secret access", realApiKeyRead:"disabled", keychainAccess:"disabled", safeStorageAccess:"disabled" }));
    ...
    expect(contracts.authContract).toEqual(expect.objectContaining({ version:"2.1.26", status:"local auth evidence only", mode:"no cloud auth", localRegister:"enabled", localLogin:"enabled", localRecoveryNotice:"enabled", passwordVerifier:"enabled", legacyPlainPasswordMigration:"compatible" }));
  ...
  test("v2.1.26 global procurement user-facing result cards stay offline and category-specific", async () => {
    await resetCommerceTasks(page);
    await gotoRoute(page, "commerce");
    ...
    expect(contracts.checklist).toEqual(expect.objectContaining({ checklistVersion:"2.1.26", status:"draft only", mode:"local planning only", realProvider:"disabled", realNetwork:"disabled", redacted:true }));
    expect(contracts.guidance).toEqual(expect.objectContaining({ guidanceVersion:"2.1.26", status:"safe guidance only", mode:"no transaction", realProvider:"disabled", realNetwork:"disabled", payment:"disabled", order:"disabled", redacted:true }));
    expect(contracts.policy).toEqual(expect.objectContaining({ policyVersion:"2.1.26", status:"manual external search only", autoClick:"disabled", bookingUrl:"disabled", realProvider:"disabled", realNetwork:"disabled", redacted:true, allowExternalSearch:true }));
    expect(contracts.blockedPolicy.allowExternalSearch).toBe(false);
    expect(contracts.flightDetail).toEqual(expect.objectContaining({ detailQualityVersion:"2.1.26", title:"机票搜索计划", emptyResultLine:"暂无真实价格结果", currentStatusLine:"当前为离线采购规划 / 只整理条件 / 不接真实平台。", redacted:true }));
    ...
    expect(contracts.flightCard).toEqual(expect.objectContaining({ cardVersion:"2.1.26", title:"机票搜索计划", categoryLabel:"机票", currentStatusLine:"仅整理搜索条件，暂无真实价格", redacted:true }));
    ...
    expect(contracts.resultCardRules).toEqual(expect.objectContaining({ rulesVersion:"2.1.26", status:"user-facing summary only", realProvider:"disabled", realNetwork:"disabled", realPrice:"disabled", bookingUrl:"disabled", payment:"disabled", order:"disabled", identityUpload:"disabled", redacted:true }));
  ...
  test("v2.1.26 global procurement decision workspace stays offline and visible", async () => {
    await resetCommerceTasks(page);
    await gotoRoute(page, "home");
  ...
  test("v2.1.26 ticket activity result stays isolated and keeps readonly provider result schema gate", async () => {
    await resetCommerceTasks(page);
    await gotoRoute(page, "home");
  ...
  test("v2.1.26 commerce normal category smoke stays result-card only @commerce-smoke", async () => {
    await resetCommerceTasks(page);
    await gotoRoute(page, "home");
  ...
  test("v2.1.26 secure API key storage console stays metadata-only and offline @commerce-smoke", async () => {
    await resetCommerceTasks(page);
    await gotoRoute(page, "home");
  ...
  test("v2.1.26 secure API key storage test actions never reveal plaintext @commerce-smoke", async () => {
    await resetCommerceTasks(page);
    await gotoRoute(page, "home");
  ...
  test("v2.1.26 restricted request smoke strips procurement actions @commerce-smoke", async () => {
    await resetCommerceTasks(page);
    await gotoRoute(page, "home");
  ...
  test("v2.1.26 latest command priority keeps newest ticket task active and topmost @latest-command-priority", async () => {
    await resetCommerceTasks(page);
    await gotoRoute(page, "home");
  ...
  test("v2.1.26 history detail viewing does not intercept newly executed task @latest-command-priority", async () => {
    await resetCommerceTasks(page);
    await gotoRoute(page, "home");
  ...
  test("v2.1.26 latest flight request is not overridden by older ticket task @latest-command-priority", async () => {
    await resetCommerceTasks(page);
    await gotoRoute(page, "home");
  ...
  test("v2.1.26 clear finished preserves latest draft and executes newest input @latest-command-priority", async () => {
    await resetCommerceTasks(page);
    await gotoRoute(page, "home");
