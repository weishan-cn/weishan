...
test.describe.serial("commerce agent workbench", () => {
  ...
  test("v2.1.76 flight workflow release readiness dashboard stays local @commerce-smoke", async () => {
    await resetCommerceTasks(page);
  ...
  test("v2.1.76 pilot onboarding guard appears before guided test @commerce-smoke", async () => {
    await resetCommerceTasks(page);
  ...
  test("v2.1.76 read-only consent gates pilot entry locally @commerce-smoke", async () => {
    await resetCommerceTasks(page);
    await installOpenExternalMock(page);
  ...
  test("v2.1.76 pilot support issue intake stays local @commerce-smoke", async () => {
    await resetCommerceTasks(page);
    await installOpenExternalMock(page);
    const summary = await createCommerceWorkbenchDetail(page, runId + "-V2175-SUPPORT 购买7月15日上海到成都最便宜的直达机票");
    for (const text of ["只读试点问题反馈", "只读试点问题复核", "问题分流面板", "问题类型", "问题状态", "分流建议", "试点影响", "建议处理", "看不懂候选证据", "平台页面与候选证据不一致", "安全说明不清楚", "只读范围确认无法完成", "反馈填写异常", "问题反馈已脱敏", "问题可用于改进参考", "问题复核只用于改进只读候选证据流程", "不会提交客服工单或交易请求", "不代表客服工单、交易请求或出票请求"] ) {
      await expect(summary).toContainText(text, { timeout:15000 });
    }
  ...
  test("v2.1.76 restricted category blocks pilot onboarding @commerce-smoke", async () => {
    await resetCommerceTasks(page);
    ...
    await expect(summary).toContainText(/暂不可进入只读试点|安全限制/);
