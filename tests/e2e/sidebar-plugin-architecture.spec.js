...
test.describe.serial("sidebar plugin architecture", () => {
  ...
  test("plugin center is always reachable while disabled workspaces remain guarded", async () => {
    ...
    const videoCard = page.locator('[data-plugin-id="weishan.studio.video"]');
    await expect(videoCard).toContainText("制作短视频");
    await expect(videoCard).toContainText("Weishan 视频工作室");
    await expect(videoCard).toContainText("规划中 · 尚不可安装");
    await expect(videoCard).toContainText("尚未提供真实视频渲染");
    await expect(videoCard).not.toContainText("video.generate");
    await expect(videoCard).not.toContainText("plugin.video");
    await expect(videoCard).not.toContainText("Codex");
