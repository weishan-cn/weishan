...
## v2.1.4：Read-only Provider Result Schema Gate / 只读 provider 结果结构闸门
v2.1.4 新增 `commerceReadonlyProviderResultSchemaGate.js`，只建立只读 provider result schema gate 草案闸门。该闸门用于展示 future-only result 类型草案、字段 allowlist、字段 blocklist、价格完整性规则、来源完整性规则、bookingUrl 规则、raw provider payload 规则、result 风险扫描草案和 result 审计事件草案。当前版本仍不读取真实 provider result，不连接真实 provider，不连接真实 endpoint，不发起真实网络请求，不读取真实 API key，不显示真实价格，不显示 availability，不显示 bookingUrl，不展示 raw provider payload，不预订、不付款、不下单、不上传身份资料。
...
## v2.1.4：Provider Result Source Label + Price Integrity Gate Bundle

v2.1.4 新增 `commerceProviderResultSourceLabelGate.js` 和 `commercePriceIntegrityTaxesFeesGate.js`。本阶段只建立未来只读 provider result 来源标签闸门和价格完整性 / 税费完整性闸门。两个 gate 只展示结构草案、UI 文案、安全规则、风险扫描草案和审计事件草案；当前版本仍不读取真实 provider result，不连接真实 endpoint，不联网，不读取或保存真实 API key，不显示真实价格，不显示 availability，不生成 bookingUrl，不预订、不付款、不下单。

`查看 provider result source label gate` 默认折叠。展开后必须显示：gate 已建立、status: closed、mode: draft only、real provider source label 未开放、real provider result 未读取、real network disabled。未来 source label 必填字段草案必须包含 providerId、providerName、sourceType、sourceUrlHost、sourceHostDisplayName、providerRegion、updatedAt、resultObservedAt、readonlyEvidence、evidenceType、sourceTrustState、redacted: true。sourceType 草案必须包含 user_bound_api、weishan_readonly_provider、public_search、manual_reviewed_source、blocked_unknown_source、no_provider。用户可见来源标签草案必须显示来源未接入真实 provider、Provider 未绑定 / 未连接、Source host 未连接真实来源、Updated at 无真实更新时间、Evidence readonlyEvidence draft only、Trust state closed / pending review。

source label 阻断规则必须显示：缺 providerId 阻断、缺 providerName 阻断、缺 sourceUrlHost 阻断、缺 updatedAt 阻断、缺 readonlyEvidence 阻断、unknown host 阻断、short URL 阻断、credential query params 阻断、token / apiKey / secret 参数阻断、raw provider URL with secrets 阻断、raw provider payload 阻断。审计事件草案必须显示 sourceLabelAuditDraft、eventType、schemaVersion、gateState、blockedReason、sourceUrlHost、resultObservedAt、redacted: true。联动必须显示只读 provider result schema gate、只读 provider sandbox gate、provider endpoint allowlist gate、key 生命周期、密钥脱敏规则、本机安全存储、API 绑定准备状态。

`查看 price integrity / taxes / fees gate` 默认折叠。展开后必须显示：gate 已建立、status: closed、mode: draft only、real price display disabled、real provider price disabled、tax / fee verification disabled until readonly provider result is available。未来 price quote 必填字段草案必须包含 providerId、providerName、sourceUrlHost、currency、baseFare、taxes、fees、total、priceObservedAt、updatedAt、readonlyEvidence、taxFeeCompleteness、quoteType、redacted: true。

价格显示前置条件必须显示：没有 providerId 不显示价格、没有 providerName 不显示价格、没有 sourceUrlHost 不显示价格、没有 currency 不显示价格、没有 total 不显示价格、没有 taxes / fees 完整性信息不显示价格、没有 updatedAt 不显示价格、没有 readonlyEvidence 不显示价格、没有 source label gate 通过不显示价格、没有 result schema gate 通过不显示价格。当前价格策略必须显示：当前版本仍隐藏价格、当前只显示“暂无真实价格结果”、当前不得显示 fake price、mock price、demo price、AI 估价、最低价 / 约 ¥xxx / estimated price，且不得根据不完整来源计算最低价。

税费完整性规则必须显示：baseFare、taxes、fees、total 必须可追溯；税费缺失、税费未知、币种缺失、更新时间缺失、provider evidence 缺失或 source label 缺失时均 price withheld。风险扫描草案必须显示 priceIntegrityRiskScanDraft、missingCurrency、missingTaxes、missingFees、missingUpdatedAt、missingReadonlyEvidence、untrustedSourceHost、estimatedPriceDetected、mockPriceDetected、bookingUrlDetected、rawProviderPayloadDetected、redacted: true。审计事件草案必须显示 priceIntegrityAuditDraft、eventType、schemaVersion、gateState、withheldReason、providerId、sourceUrlHost、priceObservedAt、taxFeeCompleteness、redacted: true。

marker:provider result source label gate
marker:provider result source label gate closed
marker:provider result source label draft only
marker:provider result source label no real provider result
marker:provider result source label no network
marker:provider result source label no price
marker:provider result source label no booking url
marker:provider result source label required fields
marker:provider result source label source types
marker:provider result source label blocked sources
marker:provider result source label audit redacted

marker:price integrity taxes fees gate
marker:price integrity taxes fees gate closed
marker:price integrity taxes fees draft only
marker:price integrity taxes fees no real price
marker:price integrity taxes fees no provider price
marker:price integrity taxes fees no booking url
marker:price integrity taxes fees required fields
marker:price integrity taxes fees prerequisites
...
## v2.1.9：Offline Provider Enforcement Engine + Evidence Pack / 离线 provider 执行闸门与证据包
...
## v2.1.15：Global Procurement Multi-Category Planner / 全球采购多品类规划

v2.1.15 新增 `globalProcurementIntentRouter.js`、`globalProcurementPlanComposer.js`、`globalProcurementRestrictedCategoryGuard.js` 和 `globalProcurementEvidenceSafetySummary.js`。本阶段只做多品类 intent routing、离线采购计划、受限品类本地安全闸门和安全证据摘要；仍不接真实 provider，不读取 API key，不连接 endpoint，不联网，不显示真实价格，不生成 bookingUrl，不预订，不付款，不下单，不上传或保存身份证、护照或银行卡。

- marker:global procurement multi-category intent router
- marker:global procurement plan composer
- marker:global procurement restricted category guard
- marker:global procurement evidence safety summary
- marker:global procurement no real provider
- marker:global procurement no real api key
- marker:global procurement no endpoint
- marker:global procurement no network
- marker:global procurement no price
...
v2.1.36 新增 `commerceGlobalProcurementDecisionWorkspace.js`。本阶段把全球采购结果进一步整理为“决策工作台”：默认只展示采购决策维度、决策规则、候选 schema、推荐模板、执行边界、风险提示、下一步与联动关系，强调当前只做离线采购决策整理，不连接真实 provider，不读取 API key，不连接 endpoint，不联网，不显示真实价格，不生成 bookingUrl，不预订，不付款，不下单，不上传或保存身份证、护照或银行卡。
...
v2.1.36 新增 `providerConnectionReadinessConsole.js` 和 `providerConnectionReadinessDecisionEngine.js`。本阶段只做 Provider 接入准备状态汇总、离线 go/no-go 决策矩阵和审计草案展示；仍不接真实 provider，不读取或保存 API key，不连接 endpoint，不联网，不显示真实价格，不显示 availability，不生成 bookingUrl，不预订，不付款，不下单，不上传或保存身份证、护照或银行卡。
...
## v2.1.36：Secure API Key Storage Implementation / 安全 API Key 存储实现

v2.1.36 新增主进程 `secureApiKeyStorage.js` 和渲染层 `commerceSecureApiKeyStorageConsole.js`。本阶段只验证本机加密存储能力：使用 Electron safeStorage 可用时加密测试占位凭据，落盘文件为 `secure-provider-credentials.v1.json.enc`，返回 UI 的内容仅限 metadata only，不展示、不导出、不复制、不记录明文。safeStorage 不可用时必须返回 storage unavailable，plaintextFallback:false，不允许明文 fallback。
...
v2.1.36 新增 `credentialConsentScopeGate.js`、`readOnlyProviderAdapterContract.js` 和 `flightReadOnlyProviderAdapterV1.js`。本阶段只允许建立授权范围草案、只读 provider adapter 合同和机票离线 fixture adapter；不连接真实 provider，不读取或保存真实 API key，不连接 endpoint，不发起网络请求，不返回真实价格、availability 或 bookingUrl，不创建订单，不付款，不上传身份信息。

- marker:credential consent scope gate v2.1.36 allowed scopes
...
- 查看 Provider Sandbox Real-Key Dry Run Gate
- endpoint allowlist enforcement only
- sandbox allowlist only
- sandbox real-key dry-run gate only
...
- marker:v2.1.36 endpoint sandbox gates home ui
- marker:v2.1.36 endpoint sandbox gates detail ui
- marker:endpoint allowlist enforcement core test
- marker:provider sandbox real-key dry run gate core test
- marker:v2.1.36 endpoint sandbox gates e2e
...
v2.1.36 新增 `providerSandboxResponseSchemaGate.js`、`realProviderResultSchemaValidation.js` 和 `providerResultSourceLabelGate.js`。本阶段只验证 sandbox response schema、真实 provider-shaped result schema 与 source label，不把任何 provider 结果放入普通结果面；不展示真实价格、availability、bookingUrl、raw payload、headers、body、证件、银行卡、付款或下单入口。
...
- marker:v2.1.36 schema source gates home ui
- marker:v2.1.36 schema source gates detail ui
...
- marker:v2.1.36 schema source gates e2e
...
- redacted: true
- marker:price integrity taxes fees gate v1
...
- marker:v2.1.36 price integrity real display home ui
- marker:v2.1.36 price integrity real display detail ui
- marker:price integrity taxes fees gate v1 core test
- marker:real price display gate core test
- marker:v2.1.36 price integrity real display e2e
...
## v2.1.36：Manual Provider Review Workflow V1 + Limited Real Price UI Beta

v2.1.36 新增 `manualProviderReviewWorkflowV1.js` 和 `limitedRealPriceUiBetaGate.js`。本阶段只允许 `flight_provider` / `flight` 在本地人工审查对象全部通过时进入 `approved_for_limited_beta`，并只允许普通机票结果面展示 `Limited Beta · 已验证只读价格` 的只读价格卡。该卡仍为受控 Beta 展示，不代表生产 provider 已接入，不允许 bookingUrl、付款、下单、身份上传或任何写入动作。

- v2.1.36：Manual Provider Review Workflow V1 + Limited Real Price UI Beta
- manualProviderReviewWorkflowV1.js
- limitedRealPriceUiBetaGate.js
- 查看 Manual Provider Review Workflow V1
- 查看 Limited Real Price UI Beta Gate
...
- no bookingUrl
- no identity upload
- limited_real_price_ui_beta_gate
...
- guardedBetaPriceDisplayedCount
...
- identityUploadAttemptCount: 0
- redacted: true
- marker:manual provider review workflow v1
- marker:manual provider review workflow v1 counters safe
...
- marker:v2.1.36 limited beta home ui
- marker:v2.1.36 limited beta detail ui
- marker:manual provider review workflow v1 core test
- marker:limited real price ui beta gate core test
- marker:v2.1.36 limited beta e2e
...
## v2.1.36：Limited Beta Kill Switch + Rollback Guard + Manual Booking Handoff

v2.1.36 新增 `limitedBetaKillSwitch.js`、`limitedBetaRollbackGuard.js` 和 `manualBookingHandoff.js`。本阶段只为 flight limited beta 增加本地 kill switch、强制回滚闸门和人工核对交接清单；当 kill switch 关闭或 rollback_active 时，普通结果面必须退回“暂无真实价格结果”，并隐藏受控价格证据。Manual Booking Handoff 只允许用户复制人工核对清单，不自动打开外部页面，不生成 bookingUrl，不付款，不下单，不上传证件或银行卡。

- v2.1.36：Limited Beta Kill Switch + Rollback Guard + Manual Booking Handoff
- limitedBetaKillSwitch.js
- limitedBetaRollbackGuard.js
- manualBookingHandoff.js
- 查看 Limited Beta Kill Switch
- 查看 Limited Beta Rollback Guard
- 查看 Manual Booking Handoff
- Limited Beta Kill Switch
- Limited Beta Rollback Guard
- Manual Booking Handoff
- globalLimitedBetaEnabled
- flight beta: true
- product beta: false
- hotel beta: false
...
- identityUploadAttemptCount: 0
- redacted: true
- marker:limited beta kill switch
- marker:limited beta kill switch flight only
- marker:limited beta rollback guard
...
- marker:v2.1.36 limited beta kill switch home ui
- marker:v2.1.36 limited beta kill switch detail ui
...
- marker:v2.1.36 limited beta kill switch e2e
...
## v2.1.36：Top 3 Result Cards + Provider Handoff UI

v2.1.36 新增 `topResultCardsBuilder.js`、`providerHandoffUi.js`、`cleanResultSurfaceV2.js`、`flightFareBreakdown.js` 和 `cheapestTruthGuard.js`。本阶段只把清晰采购请求收敛为最多 3 张用户可读结果卡，并提供手动“去平台确认”交接面板；不会为了凑数量伪造第二/第三条结果，不生成 bookingUrl，不自动打开外部平台，不付款、不下单、不上传身份证、护照或银行卡。Limited Beta 价格必须清楚标注为“只读验证价 / 非生产成交价”，并展示票面价、燃油附加费、机场建设费 / 民航发展基金、平台服务费、税费、其它附加费、优惠 / 补贴和最终应付总价。Limited Beta、Sandbox/Test、税费不完整或来源未通过的结果不得声称“真实最低价”，不得参与生产真实最低价排序；最终价格、库存、税费、行李和退改签以平台页面为准。

- marker:top result cards builder
- marker:top result cards safety counters
- marker:flight fare breakdown
- marker:flight fare breakdown user rows
- marker:cheapest truth guard
- marker:cheapest truth guard beta blocked
- marker:top result cards cheapest truth audit
- marker:provider handoff ui manual only
- marker:provider handoff fare copy payload
- marker:clean result surface v2
- marker:clean result surface v2 cheapest truth
- marker:v2.1.36 top cards handoff debug home ui
- marker:v2.1.36 top cards handoff debug detail ui
- marker:v2.1.36 top result cards core test
- marker:v2.1.36 flight fare breakdown core test
- marker:v2.1.36 cheapest truth guard core test
- marker:v2.1.36 provider handoff core test
- marker:v2.1.36 clean result surface v2 core test
- marker:v2.1.36 no fake mock demo ai price
- marker:v2.1.36 no booking url payment order identity upload

## v2.1.36：Flight Intent Parser Polish + Fare Card UX Cleanup

v2.1.36 继续保持不接生产 provider、不生成 bookingUrl、不付款、不下单、不上传证件或银行卡，只修用户面机票解析和结果卡展示。机票意图解析必须把“直达 / 直飞 / 不转机 / 最便宜 / 最低价”等修饰词从城市字段中移除，并把它们映射为直达偏好和低价排序。普通用户面价格拆分必须使用“字段：值”的清晰格式，隐藏 `Cheapest Truth Guard`、`not_ranked_as_real_cheapest` 等内部 enum，把相关审计留在“查看安全与调试详情”。外部搜索按钮必须归入“手动核对入口”，不得变成预订链接或自动跳转付款。

- marker:flight intent parser polish
- marker:flight intent parser modifier cleanup
- marker:fare card ux cleanup audit
- marker:v2.1.36 flight intent parser polish core test
- marker:v2.1.36 fare card ux cleanup core test
