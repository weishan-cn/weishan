;(function () {
  ...
  const FLIGHT_WORKFLOW_READ_ONLY_PUBLIC_PILOT_CHECKLIST_VERSION = "2.1.77";
  ...
  function evaluateFlightWorkflowPublicPilotReadiness(input) {
    ...
    const blocked = blockedInput(safe) || scenarioMatrixReady === false || gate.status === "blocked" || issueReview.status === "blocked" || supportReadiness.status === "blocked";
