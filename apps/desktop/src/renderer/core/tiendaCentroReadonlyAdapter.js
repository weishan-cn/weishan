;(function () {
  ...
  const ATTRIBUTION_URL = "https://tiendacentro.com/";
  ...
  function status(input) {
    ...
    return {
      ...
      stageLabel:"商户公开只读数据",
      adapterVersion:VERSION + "-tienda-centro-public-readonly",
      userMessage:safe.ok === true
        ? "Tienda Centro 当前价格查询已连接；覆盖阿根廷区域商品，配送、税费与库存须在商户页面核验。"
        : "Tienda Centro 当前价格查询暂时不可用。",
  ...
  function normalizeResult(input, options) {
    ...
    const sourceResults = Array.isArray(safe.results) ? safe.results.slice(0, 3) : [];
    const candidates = sourceResults.map(function (item) {
      ...
      return {
        ...
        availabilityFreshness:{
          ...
          availabilityStatus:String(evidence.availabilityStatus || "UNKNOWN").toLowerCase(),
          freshnessLevel:evidence.evidenceFreshness === "CURRENT" ? "fresh" : "stale",
          redacted:true
        },
        dataSource:{
          sourceType:"merchant_public_readonly",
        ...
        sourceType:"tienda_centro_public_api",
        trustLevel:"high",
        isOfficial:true,
        category:"product",
        coverageRegion:"Argentina",
        ...
        riskNote:"阿根廷区域商户价格可能变化；库存、规格、配送范围与最终价格请在商户商品页核验。",
        recommendationReason:"当前有效的商户公开只读商品价格，带有精确商品身份、币种、检索时间与商户商品页。",
        responseProvenance:{
          providerIdentity:PROVIDER_ID,
          responseProvenance:"tienda_centro_woocommerce_store_api",
          sourceType:"PUBLIC_READ_ONLY",
          redacted:true
        },
  ...
  window.WeishanTiendaCentroReadonlyAdapter = Object.freeze({
    VERSION,
    PROVIDER_ID,
    PROVIDER_NAME,
    ATTRIBUTION_URL,
