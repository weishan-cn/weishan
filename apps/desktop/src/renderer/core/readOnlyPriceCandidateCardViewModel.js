;(function () {
  ...
  const READ_ONLY_PRICE_CANDIDATE_CARD_VIEW_MODEL_VERSION = "4.1.5";
  ...
  function renderReadOnlyPriceCandidateCardHtml(input) {
    ...
    const publicBetaQaOperationsHtml = card.publicBetaFreezeReviewViewModelSummary ? '<section class="commerce-global-shopping-public-beta-qa-operations" data-commerce-global-shopping-public-beta-qa-operations="true"><h5>Public Beta QA Freeze Gate</h5><p>Manual Trial Summary Board</p><p>Offline Readiness Review Panel</p><p>Frozen Scope</p><p>Allowed Next Actions</p><p>Blocked Next Actions</p><p>Manual Review Required</p><p>当前冻结的是只读 QA 范围，不执行真实 freeze</p><p>只允许继续测试、人工复核或阻断</p><p>仍不允许启用 provider、付款、下单或发布</p><p>人工复核后再决定下一阶段</p><p>' + escapeHtml(card.publicBetaQaFreezeGateSummary && card.publicBetaQaFreezeGateSummary.userFacingSummary && card.publicBetaQaFreezeGateSummary.userFacingSummary.resultLabel || 'Public Beta QA Freeze Gate 仍需复核') + '</p><p>' + escapeHtml(card.manualTrialSummaryBoardSummary && card.manualTrialSummaryBoardSummary.userFacingSummary && card.manualTrialSummaryBoardSummary.userFacingSummary.resultLabel || 'Manual Trial Summary Board 仍需复核') + '</p><p>' + escapeHtml(card.offlineReadinessReviewPanelSummary && card.offlineReadinessReviewPanelSummary.userFacingSummary && card.offlineReadinessReviewPanelSummary.userFacingSummary.resultLabel || 'Offline Readiness Review Panel 仍需复核') + '</p><p>' + escapeHtml(card.publicBetaFreezeReviewViewModelSummary && card.publicBetaFreezeReviewViewModelSummary.userFacingSummary && card.publicBetaFreezeReviewViewModelSummary.userFacingSummary.resultLabel || 'Public Beta Freeze Review View Model 仍需复核') + '</p><p>manualReviewRequired:true</p><p>safeToProceedWithManualFreezeReview:' + escapeHtml(String(card.safeToProceedWithManualFreezeReview === true)) + '</p><p>externalUrl:null</p><p>platformUrl:null</p><p>providerUrl:null</p><p>bookingUrl:null</p><p>checkoutUrl:null</p><p>paymentUrl:null</p><p>orderUrl:null</p><p>buyButtonEnabled:false</p><p>checkoutButtonEnabled:false</p><p>paymentButtonEnabled:false</p></section>' : '';
