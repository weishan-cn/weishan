;(function () {
  ...
  const READ_ONLY_PRICE_CANDIDATE_CARD_VIEW_MODEL_VERSION = "2.3.6";
  ...
  function buildReadOnlyPriceCandidateCardViewModel(input) {
    ...
    const providerSandboxDryRunViewModelSummary = safe.providerSandboxDryRunViewModelSummary && typeof safe.providerSandboxDryRunViewModelSummary === "object" ? safe.providerSandboxDryRunViewModelSummary : (typeof globalShoppingProviderSandboxDryRunViewModelApi.buildGlobalShoppingProviderSandboxDryRunViewModel === "function" ? globalShoppingProviderSandboxDryRunViewModelApi.buildGlobalShoppingProviderSandboxDryRunViewModel({
    ...
    const providerSandboxDryRunViewModelStatus = text(providerSandboxDryRunViewModelSummary && providerSandboxDryRunViewModelSummary.status || "");
    ...
    const safeToProceedWithHumanAuditSandboxPilotReadinessReview = providerPilotGovernanceViewModelStatus === "ready";
    ...
    return clone({
      ...
      safeToProceedWithHumanControlledSandboxProviderPilot:safeToProceedWithHumanControlledSandboxProviderPilot, safeToProceedWithHumanControlledSandboxProviderPilotPlan:safeToProceedWithHumanControlledSandboxProviderPilotPlan, safeToProceedWithHumanAuditSandboxPilotReadinessReview:safeToProceedWithHumanAuditSandboxPilotReadinessReview,
      rcReviewStatus: rcReviewStatus,
      rcEvidenceStatus: rcEvidenceStatus,
      rcRegressionStatus: rcRegressionStatus,
      releaseRiskStatus: releaseRiskStatus,
  ...
  function renderReadOnlyPriceCandidateCardHtml(input) {
    ...
    const providerSandboxDryRunHtml = card.providerSandboxDryRunViewModelSummary || card.providerAdapterRegistryViewModelSummary ? '<section class="commerce-global-shopping-provider-sandbox-dry-run" data-commerce-global-shopping-provider-sandbox-dry-run="true"><h5>Provider Sandbox 干跑准备</h5><p>Provider Sandbox 干跑框架</p><p>第一个只读 Provider Adapter 外壳</p><p>Provider Sandbox 安全熔断器</p><p>Provider Adapter 注册与接入手册</p><p>Provider Adapter 注册表</p><p>Dry-Run Provider 响应归一化器</p><p>Sandbox Provider 接入运行手册</p><p>干跑框架</p><p>Adapter 外壳</p><p>安全熔断器</p><p>Provider Sandbox 干跑框架已准备</p><p>第一个只读 Provider Adapter 外壳已准备</p><p>Provider Sandbox 安全熔断器未触发</p><p>Adapter 注册表已准备</p><p>Dry-run 响应归一化已准备</p><p>Sandbox Provider 接入手册已准备</p><p>只允许只读 adapter 注册</p><p>不接收 raw provider response</p><p>接入手册不执行真实接入</p><p>Adapter 注册不代表真实 provider 接通</p><p>当前仅管理只读 fixture/dry-run/sandbox adapter</p><p>不包含真实 endpoint、真实密钥、真实网络调用或下单能力</p><p>干跑不发送真实请求</p><p>Adapter 外壳不包含真实 endpoint</p><p>安全熔断器阻断真实 provider 风险</p><p>干跑不代表真实价格或下单能力</p><p>当前仅模拟只读 provider sandbox 生命周期</p><p>不发送请求，不读取真实密钥，不保存 raw request 或 raw response</p><button type="button" class="cmd-btn gray" data-commerce-global-shopping-provider-dry-run-show="true">查看干跑框架</button><button type="button" class="cmd-btn gray" data-commerce-global-shopping-adapter-shell-show="true">查看 Adapter 外壳</button><button type="button" class="cmd-btn gray" data-commerce-global-shopping-kill-switch-show="true">查看安全熔断器</button><button type="button" class="cmd-btn gray" data-commerce-global-shopping-provider-adapter-registry-show="true">查看 Adapter 注册表</button><button type="button" class="cmd-btn gray" data-commerce-global-shopping-provider-response-normalizer-show="true">查看响应归一化</button><button type="button" class="cmd-btn gray" data-commerce-global-shopping-provider-runbook-show="true">查看接入手册</button><div data-commerce-global-shopping-provider-dry-run-output="true"><p>Provider Sandbox 干跑框架</p><p>' + escapeHtml(card.providerSandboxDryRunHarnessSummary && card.providerSandboxDryRunHarnessSummary.userFacingSummary && card.providerSandboxDryRunHarnessSummary.userFacingSummary.resultLabel || 'Provider Sandbox 干跑框架仍需复核') + '</p><p>干跑不发送真实请求</p><p>bookingUrl:null</p><p>payment:false</p><p>order:false</p></div><div data-commerce-global-shopping-adapter-shell-output="true"><p>第一个只读 Provider Adapter 外壳</p><p>' + escapeHtml(card.firstReadOnlyProviderAdapterShellSummary && card.firstReadOnlyProviderAdapterShellSummary.userFacingSummary && card.firstReadOnlyProviderAdapterShellSummary.userFacingSummary.resultLabel || '第一个只读 Provider Adapter 外壳仍需复核') + '</p><p>Adapter 外壳不包含真实 endpoint</p><p>bookingUrl:null</p><p>payment:false</p><p>order:false</p></div><div data-commerce-global-shopping-kill-switch-output="true"><p>Provider Sandbox 安全熔断器</p><p>' + escapeHtml(card.providerSandboxSafetyKillSwitchSummary && card.providerSandboxSafetyKillSwitchSummary.userFacingSummary && card.providerSandboxSafetyKillSwitchSummary.userFacingSummary.resultLabel || '安全熔断器阻断真实 provider 风险') + '</p><p>安全熔断器阻断真实 provider 风险</p><p>bookingUrl:null</p><p>payment:false</p><p>order:false</p></div><div data-commerce-global-shopping-provider-adapter-registry-output="true"><p>Provider Adapter 注册表</p><p>' + escapeHtml(card.providerAdapterRegistrySummary && card.providerAdapterRegistrySummary.userFacingSummary && card.providerAdapterRegistrySummary.userFacingSummary.resultLabel || 'Adapter 注册表仍需复核') + '</p><p>只允许只读 adapter 注册</p><p>当前仅管理只读 fixture/dry-run/sandbox adapter</p><p>bookingUrl:null</p><p>payment:false</p><p>order:false</p></div><div data-commerce-global-shopping-provider-response-normalizer-output="true"><p>Dry-Run Provider 响应归一化器</p><p>' + escapeHtml(card.dryRunProviderResponseNormalizerSummary && card.dryRunProviderResponseNormalizerSummary.userFacingSummary && card.dryRunProviderResponseNormalizerSummary.userFacingSummary.resultLabel || '响应归一化仍需复核') + '</p><p>不接收 raw provider response</p><p>不包含真实 endpoint、真实密钥、真实网络调用或下单能力</p><p>bookingUrl:null</p><p>payment:false</p><p>order:false</p></div><div data-commerce-global-shopping-provider-runbook-output="true"><p>Sandbox Provider 接入运行手册</p><p>' + escapeHtml(card.sandboxProviderRunbookSummary && card.sandboxProviderRunbookSummary.userFacingSummary && card.sandboxProviderRunbookSummary.userFacingSummary.resultLabel || '接入手册仍需复核') + '</p><p>接入手册不执行真实接入</p><p>Adapter 注册不代表真实 provider 接通</p><p>bookingUrl:null</p><p>payment:false</p><p>order:false</p></div></section>' : '';
