;(function () {
  ...
  const READ_ONLY_PRICE_CANDIDATE_CARD_VIEW_MODEL_VERSION = "3.0.0";
  ...
  function renderReadOnlyPriceCandidateCardHtml(input) {
    ...
    const providerLaunchControlHtml = card.providerLaunchControlViewModelSummary ? '<section class="commerce-global-shopping-provider-launch-control" data-commerce-global-shopping-provider-launch-control="true"><h5>Provider Launch Control Tower</h5><p>Offline Provider Launch Control Tower</p><p>Adapter Policy Engine</p><p>Human Release Evidence Timeline</p><p>Sandbox Activation Final Review Board</p><p>Launch Control</p><p>Adapter Policy</p><p>Evidence Timeline</p><p>Final Review</p><p>' + escapeHtml(card.offlineProviderLaunchControlTowerSummary && card.offlineProviderLaunchControlTowerSummary.userFacingSummary && card.offlineProviderLaunchControlTowerSummary.userFacingSummary.resultLabel || '离线 Launch 控制仍需复核') + '</p><p>' + escapeHtml(card.adapterPolicyEngineSummary && card.adapterPolicyEngineSummary.userFacingSummary && card.adapterPolicyEngineSummary.userFacingSummary.resultLabel || 'Adapter 策略仍需复核') + '</p><p>' + escapeHtml(card.humanReleaseEvidenceTimelineSummary && card.humanReleaseEvidenceTimelineSummary.userFacingSummary && card.humanReleaseEvidenceTimelineSummary.userFacingSummary.resultLabel || '人工发布证据仍需复核') + '</p><p>' + escapeHtml(card.sandboxActivationFinalReviewBoardSummary && card.sandboxActivationFinalReviewBoardSummary.userFacingSummary && card.sandboxActivationFinalReviewBoardSummary.userFacingSummary.resultLabel || 'Sandbox 激活终审仍需复核') + '</p><p>Launch Control 不保存真实决策</p><p>Adapter Policy 不修改配置、不启用 provider</p><p>Evidence Timeline 不持久化时间线</p><p>Final Review 不激活 sandbox</p><p>Human launch control review 仍需人工复核</p><p>当前只展示 provider launch control tower</p><p>不接真实 provider，不读取密钥，不联网，不激活 sandbox，不创建 release，不 push</p><button type="button" class="cmd-btn gray" data-commerce-global-shopping-launch-control-show="true">查看 Launch Control</button><button type="button" class="cmd-btn gray" data-commerce-global-shopping-adapter-policy-show="true">查看 Adapter Policy</button><button type="button" class="cmd-btn gray" data-commerce-global-shopping-evidence-timeline-show="true">查看 Evidence Timeline</button><button type="button" class="cmd-btn gray" data-commerce-global-shopping-final-review-show="true">查看 Final Review</button><div data-commerce-global-shopping-launch-control-output="true"><p>Offline Provider Launch Control Tower</p><p>' + escapeHtml(card.offlineProviderLaunchControlTowerSummary && card.offlineProviderLaunchControlTowerSummary.userFacingSummary && card.offlineProviderLaunchControlTowerSummary.userFacingSummary.resultLabel || '离线 Launch 控制仍需复核') + '</p><p>Launch Control 不保存真实决策</p><p>当前只展示 provider launch control tower</p></div><div data-commerce-global-shopping-adapter-policy-output="true"><p>Adapter Policy Engine</p><p>' + escapeHtml(card.adapterPolicyEngineSummary && card.adapterPolicyEngineSummary.userFacingSummary && card.adapterPolicyEngineSummary.userFacingSummary.resultLabel || 'Adapter 策略仍需复核') + '</p><p>Adapter Policy 不修改配置、不启用 provider</p><p>bookingUrl:null</p><p>payment:false</p><p>order:false</p></div><div data-commerce-global-shopping-evidence-timeline-output="true"><p>Human Release Evidence Timeline</p><p>' + escapeHtml(card.humanReleaseEvidenceTimelineSummary && card.humanReleaseEvidenceTimelineSummary.userFacingSummary && card.humanReleaseEvidenceTimelineSummary.userFacingSummary.resultLabel || '人工发布证据仍需复核') + '</p><p>Evidence Timeline 不持久化时间线</p><p>bookingUrl:null</p><p>payment:false</p><p>order:false</p></div><div data-commerce-global-shopping-final-review-output="true"><p>Sandbox Activation Final Review Board</p><p>' + escapeHtml(card.sandboxActivationFinalReviewBoardSummary && card.sandboxActivationFinalReviewBoardSummary.userFacingSummary && card.sandboxActivationFinalReviewBoardSummary.userFacingSummary.resultLabel || 'Sandbox 激活终审仍需复核') + '</p><p>Final Review 不激活 sandbox</p><p>不接真实 provider，不读取密钥，不联网，不激活 sandbox，不创建 release，不 push</p></div></section>' : '';
