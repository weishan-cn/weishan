;(function () {
  ...
  const READ_ONLY_PRICE_CANDIDATE_CARD_VIEW_MODEL_VERSION = "4.0.6";
  ...
  function renderReadOnlyPriceCandidateCardHtml(input) {
    ...
    const publicBetaOperatorReviewHtml = card.publicBetaOperatorViewModelSummary ? '<section class="commerce-global-shopping-public-beta-operator-review" data-commerce-global-shopping-public-beta-operator-review="true"><h5>Public Beta Operator Console</h5><p>Public Beta Operator Console</p><p>Category Expansion Shell</p><p>Public Beta User Journey</p><p>Safe Search Intent Matrix</p><p>User Boundary Panel</p><p>Category Result Simulator</p><p>Read-Only Comparison Board</p><p>Result Trust Badge</p><p>Final Offline Beta Audit</p><p>Public Beta Acceptance Board</p><p>Manual Review Required</p><p>' + escapeHtml(card.publicBetaOperatorConsoleSummary && card.publicBetaOperatorConsoleSummary.userFacingSummary && card.publicBetaOperatorConsoleSummary.userFacingSummary.resultLabel || 'Public Beta Operator Console 仍需复核') + '</p><p>' + escapeHtml(card.categoryExpansionShellSummary && card.categoryExpansionShellSummary.userFacingSummary && card.categoryExpansionShellSummary.userFacingSummary.resultLabel || 'Category Expansion Shell 仍需复核') + '</p><p>' + escapeHtml(card.finalOfflineBetaAuditSummary && card.finalOfflineBetaAuditSummary.userFacingSummary && card.finalOfflineBetaAuditSummary.userFacingSummary.resultLabel || 'Final Offline Beta Audit 仍需复核') + '</p><p>' + escapeHtml(card.publicBetaAcceptanceBoardSummary && card.publicBetaAcceptanceBoardSummary.userFacingSummary && card.publicBetaAcceptanceBoardSummary.userFacingSummary.resultLabel || 'Public Beta Acceptance Board 仍需复核') + '</p><p>' + escapeHtml(card.publicBetaOperatorViewModelSummary && card.publicBetaOperatorViewModelSummary.userFacingSummary && card.publicBetaOperatorViewModelSummary.userFacingSummary.resultLabel || '仍需人工复核后再决定是否进入下一阶段') + '</p><p>Flight / Hotel / Product 只读外壳已准备</p><p>Provider-Zero 状态通过</p><p>候选价证据通过</p><p>费用归一化通过</p><p>官方价锚点通过</p><p>安全文案通过</p><p>最终离线审计通过</p><p>仍需人工复核后再决定是否进入下一阶段</p><p>bookingUrl:null</p><p>payment:false</p><p>order:false</p><p>release:false</p><p>push:false</p></section>' : '';
