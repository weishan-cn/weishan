;(function () {
  ...
  const READ_ONLY_PRICE_CANDIDATE_CARD_VIEW_MODEL_VERSION = "2.1.75";
  ...
  function renderReadOnlyPriceCandidateCardHtml(input) {
    ...
    const pilotSupportHtml = '<section class="commerce-flight-pilot-support" data-commerce-flight-pilot-support="true"><h5>只读试点问题反馈</h5><p>问题类型</p><p>建议处理</p><p>看不懂候选证据</p><p>平台页面与候选证据不一致</p><p>安全说明不清楚</p><p>只读范围确认无法完成</p><p>反馈填写异常</p><p>问题反馈已脱敏</p><p>问题反馈只用于改进只读候选证据流程</p><p>不代表客服工单、交易请求或出票请求</p><p>建议重新查看候选证据</p><p>建议记录平台核对结果</p><p>建议查看安全说明</p><p>建议重新确认只读范围</p><p>bookingUrl:null</p><p>paymentUrl:null</p><p>orderUrl:null</p><p>download:false</p><p>fileWrite:false</p><button type="button" class="cmd-btn gray" data-commerce-flight-pilot-support-show="true">查看问题反馈</button><button type="button" class="cmd-btn gray" data-commerce-flight-issue-category="candidate_unclear">看不懂候选证据</button><button type="button" class="cmd-btn gray" data-commerce-flight-issue-category="platform_mismatch">平台页面与候选证据不一致</button><button type="button" class="cmd-btn gray" data-commerce-flight-issue-category="safety_copy_unclear">安全说明不清楚</button><div data-commerce-flight-pilot-support-output="true"><p>只读试点问题反馈</p><p>建议重新查看候选证据</p></div></section>';
