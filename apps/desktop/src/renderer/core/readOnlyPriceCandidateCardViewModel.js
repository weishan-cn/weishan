;(function () {
  ...
  const READ_ONLY_PRICE_CANDIDATE_CARD_VIEW_MODEL_VERSION = "2.1.91";
  ...
  function buildReadOnlyPriceCandidateCardViewModel(input) {
    ...
    const safeToProceedWithSandboxDeepLinkCandidate = safeToProceedWithDeepLinkSafetyGate === true && externalDeepLinkSafetyStatus === "safe" && searchPrefillStatus === "safe" && handoffPreviewStatus === "ready";
  ...
  function renderReadOnlyPriceCandidateCardHtml(input) {
    ...
    const globalShoppingGoalHtml = card.globalShoppingProductGoalViewModelSummary ? '<section class="commerce-global-shopping-product-goal" data-commerce-global-shopping-product-goal="true"><h5>全球购产品目标与跳转边界</h5><p>全球购产品目标</p><p>跳转至平台查看</p><p>外部平台跳转安全闸门</p><p>搜索参数预填闸门</p><p>目标平台</p><p>可带入搜索条件</p><p>平台自行下单</p><p>安全边界</p><p>当前已覆盖来源中的较低候选价</p><p>与官方价对比</p><p>已接入平台候选价</p><p>价格以跳转后平台实时页面为准</p><p>当前仅提供只读候选证据，不提供付款、下单或出票能力</p><p>Weishan 可尽量带入搜索条件，但用户需在对应平台自行确认价格、登录、填写资料并完成下单</p><p>Weishan 仅可携带非敏感搜索条件</p><p>用户需在平台自行确认价格、登录、填写资料并完成下单</p><p>不保存平台账号</p><p>不保存证件银行卡</p><p>不保存支付凭证</p><p>禁止最低价相关承诺</p><p>禁止自动下单承诺</p><p>本轮仅展示只读跳转预览，不打开真实平台</p><p>跳转预览不代表下单能力</p><p>跳转不代表交易能力</p><p>' + escapeHtml(card.globalShoppingProductGoalSummary && card.globalShoppingProductGoalSummary.userFacingSummary && card.globalShoppingProductGoalSummary.userFacingSummary.resultLabel || "产品目标仍需复核") + '</p><p>' + escapeHtml(card.jumpToPlatformBoundarySummary && card.jumpToPlatformBoundarySummary.userFacingSummary && card.jumpToPlatformBoundarySummary.userFacingSummary.resultLabel || "跳转边界仍需复核") + '</p><p>' + escapeHtml(card.externalDeepLinkSafetySummary && card.externalDeepLinkSafetySummary.userFacingSummary && card.externalDeepLinkSafetySummary.userFacingSummary.resultLabel || "跳转安全仍需复核") + '</p><p>' + escapeHtml(card.searchParameterPrefillSummary && card.searchParameterPrefillSummary.userFacingSummary && card.searchParameterPrefillSummary.userFacingSummary.resultLabel || "预填边界仍需复核") + '</p><button type="button" class="cmd-btn gray" data-commerce-global-shopping-product-goal-show="true">查看全球购产品目标</button><button type="button" class="cmd-btn gray" data-commerce-global-shopping-jump-boundary-show="true">查看跳转边界</button><button type="button" class="cmd-btn gray" data-commerce-global-shopping-deep-link-safety-show="true">查看跳转安全</button><button type="button" class="cmd-btn gray" data-commerce-global-shopping-prefill-gate-show="true">查看预填边界</button><button type="button" class="cmd-btn gray" data-commerce-global-shopping-handoff-preview-show="true">查看跳转预览</button><div data-commerce-global-shopping-product-goal-output="true"><p>全球购产品目标与跳转边界</p><p>全球购产品目标</p><p>可信候选价格</p><p>官方价格锚点</p><p>合法平台候选价</p><p>平台自行下单</p><p>当前已覆盖来源中的较低候选价</p><p>与官方价对比</p><p>已接入平台候选价</p><p>价格以跳转后平台实时页面为准</p><p>当前仅提供只读候选证据，不提供付款、下单或出票能力</p><p>Weishan 可尽量带入搜索条件，但用户需在对应平台自行确认价格、登录、填写资料并完成下单</p><p>禁止最低价相关承诺</p><p>禁止自动下单承诺</p><p>跳转不代表交易能力</p></div><div data-commerce-global-shopping-jump-boundary-output="true"><p>跳转至平台自行下单边界</p><p>Weishan 可尽量带入搜索条件，但用户需在对应平台自行确认价格、登录、填写资料并完成下单</p><p>当前仅提供只读候选证据，不提供付款、下单或出票能力</p><p>价格以跳转后平台实时页面为准</p><p>不打开外部平台</p><p>不生成交易链接</p></div><div data-commerce-global-shopping-deep-link-safety-output="true"><p>外部平台跳转安全闸门</p><p>跳转安全结构已准备</p><p>目标平台</p><p>平台自行下单</p><p>不保存平台账号</p><p>不保存证件银行卡</p><p>不保存支付凭证</p><p>bookingUrl:null</p><p>payment:false</p><p>order:false</p></div><div data-commerce-global-shopping-prefill-gate-output="true"><p>搜索参数预填闸门</p><p>预填边界安全</p><p>可带入搜索条件</p><p>Weishan 仅可携带非敏感搜索条件</p><p>用户需在平台自行确认价格、填写必要资料并完成下单</p><p>bookingUrl:null</p><p>payment:false</p><p>order:false</p></div><div data-commerce-global-shopping-handoff-preview-output="true"><p>跳转至平台查看</p><p>目标平台</p><p>可带入搜索条件</p><p>平台自行下单</p><p>安全边界</p><p>本轮仅展示只读跳转预览，不打开真实平台</p><p>跳转预览不代表下单能力</p><p>bookingUrl:null</p><p>payment:false</p><p>order:false</p></div></section>' : '';
