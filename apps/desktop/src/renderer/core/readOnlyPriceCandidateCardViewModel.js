;(function () {
  ...
  const READ_ONLY_PRICE_CANDIDATE_CARD_VIEW_MODEL_VERSION = "2.2.5";
  ...
  function buildReadOnlyPriceCandidateCardViewModel(input) {
    ...
    return clone({
      ...
      safeToProceedWithUserFacingHandoffExplanation:safeToProceedWithUserFacingHandoffExplanation, safeToProceedWithManualPlatformReview:safeToProceedWithManualPlatformReview, safeToProceedWithManualPlatformUserEducation:safeToProceedWithManualPlatformUserEducation,
      rcReviewStatus: rcReviewStatus,
      rcEvidenceStatus: rcEvidenceStatus,
      rcRegressionStatus: rcRegressionStatus,
      releaseRiskStatus: releaseRiskStatus,
  ...
  function renderReadOnlyPriceCandidateCardHtml(input) {
    ...
    const manualPlatformReviewHtml = card.manualPlatformReviewViewModelSummary ? '<section class="commerce-global-shopping-manual-platform-review" data-commerce-global-shopping-manual-platform-review="true"><h5>手动平台复核与现实检查</h5><p>手动平台复核与现实检查</p><p>手动平台复核驾驶舱</p><p>交接包接受演练</p><p>平台真实页面复核清单</p><p>手动复核驾驶舱</p><p>接受演练</p><p>平台复核清单</p><p>手动平台复核驾驶舱已准备</p><p>交接包接受演练已准备</p><p>平台真实页面复核清单已准备</p><p>接受演练不保存用户确认</p><p>平台页面才是最终依据</p><p>手动复核不代表下单能力</p><p>Weishan 不替用户做最终决定</p><p>当前只展示手动复核、接受演练和平台真实页面复核清单</p><p>不打开平台，不保存选择，不构成订单、付款授权或签名</p><button type="button" class="cmd-btn gray" data-commerce-global-shopping-manual-platform-review-cockpit-show="true">查看手动复核驾驶舱</button><button type="button" class="cmd-btn gray" data-commerce-global-shopping-handoff-acceptance-walkthrough-show="true">查看接受演练</button><button type="button" class="cmd-btn gray" data-commerce-global-shopping-platform-reality-check-board-show="true">查看平台复核清单</button><div data-commerce-global-shopping-manual-platform-review-cockpit-output="true"><p>手动平台复核驾驶舱</p><p>' + escapeHtml(card.manualPlatformReviewCockpitSummary && card.manualPlatformReviewCockpitSummary.userFacingSummary && card.manualPlatformReviewCockpitSummary.userFacingSummary.resultLabel || '手动平台复核驾驶舱仍需复核') + '</p><p>当前只展示手动复核、接受演练和平台真实页面复核清单</p></div><div data-commerce-global-shopping-handoff-acceptance-walkthrough-output="true"><p>交接包接受演练</p><p>' + escapeHtml(card.handoffAcceptanceWalkthroughSummary && card.handoffAcceptanceWalkthroughSummary.userFacingSummary && card.handoffAcceptanceWalkthroughSummary.userFacingSummary.resultLabel || '交接包接受演练仍需复核') + '</p><p>接受演练不保存用户确认</p></div><div data-commerce-global-shopping-platform-reality-check-board-output="true"><p>平台真实页面复核清单</p><p>' + escapeHtml(card.platformRealityCheckBoardSummary && card.platformRealityCheckBoardSummary.userFacingSummary && card.platformRealityCheckBoardSummary.userFacingSummary.resultLabel || '平台真实页面复核清单仍需复核') + '</p><p>平台页面才是最终依据</p><p>手动复核不代表下单能力</p><p>Weishan 不替用户做最终决定</p></div></section>' : '';
