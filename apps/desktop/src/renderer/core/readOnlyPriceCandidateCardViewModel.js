;(function () {
  ...
  const READ_ONLY_PRICE_CANDIDATE_CARD_VIEW_MODEL_VERSION = "4.1.4";
  ...
  function renderReadOnlyPriceCandidateCardHtml(input) {
    ...
    const publicBetaQaOperationsHtml = card.publicBetaQaOperationsViewModelSummary ? '<section class="commerce-global-shopping-public-beta-qa-operations" data-commerce-global-shopping-public-beta-qa-operations="true"><h5>Public Beta Trial Evidence Ledger</h5><p>QA Decision Matrix</p><p>Offline Issue Triage Board</p><p>Allowed Decisions</p><p>Blocked Decisions</p><p>Manual Review Items</p><p>Continue Testing</p><p>Manual Review Required</p><p>只允许继续测试、人工复核或阻断</p><p>问题分流仅为离线视图，不创建真实 issue</p><p>不自动发布、不启用 provider、不启用交易</p><p>' + escapeHtml(card.publicBetaTrialEvidenceLedgerSummary && card.publicBetaTrialEvidenceLedgerSummary.userFacingSummary && card.publicBetaTrialEvidenceLedgerSummary.userFacingSummary.resultLabel || 'Public Beta Trial Evidence Ledger 仍需复核') + '</p><p>' + escapeHtml(card.publicBetaQaDecisionMatrixSummary && card.publicBetaQaDecisionMatrixSummary.userFacingSummary && card.publicBetaQaDecisionMatrixSummary.userFacingSummary.resultLabel || 'QA Decision Matrix 仍需复核') + '</p><p>' + escapeHtml(card.offlineIssueTriageBoardSummary && card.offlineIssueTriageBoardSummary.userFacingSummary && card.offlineIssueTriageBoardSummary.userFacingSummary.resultLabel || 'Offline Issue Triage Board 仍需复核') + '</p><p>' + escapeHtml(card.publicBetaQaOperationsViewModelSummary && card.publicBetaQaOperationsViewModelSummary.userFacingSummary && card.publicBetaQaOperationsViewModelSummary.userFacingSummary.resultLabel || 'Public Beta QA Operations View Model 仍需复核') + '</p><p>manualReviewRequired:true</p><p>safeToProceedWithManualQaOperationsReview:' + escapeHtml(String(card.safeToProceedWithManualQaOperationsReview === true)) + '</p><p>externalUrl:null</p><p>platformUrl:null</p><p>providerUrl:null</p><p>bookingUrl:null</p><p>checkoutUrl:null</p><p>paymentUrl:null</p><p>orderUrl:null</p><p>buyButtonEnabled:false</p><p>checkoutButtonEnabled:false</p><p>paymentButtonEnabled:false</p></section>' : '';
