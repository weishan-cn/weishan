;(function () {
  ...
  const READ_ONLY_PRICE_CANDIDATE_CARD_VIEW_MODEL_VERSION = "2.1.89";
  ...
  function renderReadOnlyPriceCandidateCardHtml(input) {
    ...
    const priceCandidateDisplayHtml = card.priceCandidateDisplaySummary ? '<section class="commerce-global-shopping-price-candidate-display" data-commerce-global-shopping-price-candidate-display="true"><h5>全球购价格候选展示</h5><p>价格源归一化层</p><p>官方价格锚点</p><p>官方参考价</p><p>已覆盖来源中的较低候选价</p><p>与官方价对比</p><p>税费/运费/服务费状态</p><p>来源与可信度</p><p>价格以跳转后平台实时页面为准</p><p>当前仅展示只读 fixture 候选价</p><p>不代表真实最终价、锁价、最低价或可下单能力</p><p>价格展示不代表下单能力</p><button type="button" class="cmd-btn gray" data-commerce-global-shopping-price-normalization-show="true">查看价格归一化</button><button type="button" class="cmd-btn gray" data-commerce-global-shopping-official-anchor-show="true">查看官方价格锚点</button><div data-commerce-global-shopping-price-normalization-output="true"><p>价格源归一化层</p><p>' + escapeHtml(card.priceSourceNormalizationSummary && card.priceSourceNormalizationSummary.userFacingSummary && card.priceSourceNormalizationSummary.userFacingSummary.resultLabel || '价格归一化仍需复核') + '</p><p>fixtureOnly / sandboxOnly / readOnly</p><p>noRealProvider:true</p><p>noNetwork:true</p></div><div data-commerce-global-shopping-official-anchor-output="true"><p>官方价格锚点</p><p>' + escapeHtml(card.officialPriceAnchorSummary && card.officialPriceAnchorSummary.userFacingSummary && card.officialPriceAnchorSummary.userFacingSummary.resultLabel || '官方价仍需复核') + '</p><p>官方价格只作为参考锚点</p><p>当前仅比较已覆盖来源中的候选价</p></div></section>' : '';
