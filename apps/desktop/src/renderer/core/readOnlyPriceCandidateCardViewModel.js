;(function () {
  ...
  const READ_ONLY_PRICE_CANDIDATE_CARD_VIEW_MODEL_VERSION = "2.2.2";
  ...
  function renderReadOnlyPriceCandidateCardHtml(input) {
    ...
    const providerCoverageHtml = card.providerCoverageViewModelSummary ? '<section class="commerce-global-shopping-provider-coverage" data-commerce-global-shopping-provider-coverage="true"><h5>Provider 覆盖与来源可信度</h5><p>第一个 Sandbox Provider Connector</p><p>Provider 覆盖看板</p><p>只读来源可信度评分</p><p>只读 Provider Sandbox 接入闸门</p><p>Sandbox 价格候选会话</p><p>Sandbox 价格候选结果</p><p>官方参考价</p><p>已覆盖来源较低候选价</p><p>跳转预览</p><p>Sandbox Connector</p><p>来源覆盖</p><p>来源可信度</p><p>第一个 Sandbox Connector 已准备</p><p>Provider 覆盖结构已准备</p><p>来源可信度评分已准备</p><p>可以创建只读 Sandbox 价格候选会话</p><p>Sandbox 价格候选会话已准备</p><p>当前仅展示只读 sandbox 候选结果，不代表真实价格、全网最低、最低价保证、锁价、可订、付款、下单或出票能力。</p><p>覆盖来源不代表全网覆盖</p><p>可信度不代表官方背书</p><p>低价不等于最佳</p><p>Provider 覆盖不代表下单能力</p><p>当前仅展示 fixture/dry-run/sandbox provider 覆盖和来源可信度</p><p>不代表全网覆盖、官方背书、真实价格或下单能力</p><button type="button" class="cmd-btn gray" data-commerce-global-shopping-first-sandbox-provider-connector-show="true">查看 Sandbox Connector</button><button type="button" class="cmd-btn gray" data-commerce-global-shopping-provider-coverage-show="true">查看 Provider 覆盖</button><button type="button" class="cmd-btn gray" data-commerce-global-shopping-source-trust-show="true">查看来源可信度</button><div data-commerce-global-shopping-first-sandbox-provider-connector-output="true"><p>第一个 Sandbox Provider Connector</p><p>' + escapeHtml(card.firstSandboxProviderConnectorSummary && card.firstSandboxProviderConnectorSummary.userFacingSummary && card.firstSandboxProviderConnectorSummary.userFacingSummary.resultLabel || 'Sandbox Connector 仍需复核') + '</p><p>bookingUrl:null</p><p>payment:false</p><p>order:false</p></div><div data-commerce-global-shopping-provider-coverage-output="true"><p>Provider 覆盖看板</p><p>' + escapeHtml(card.providerCoverageDashboardSummary && card.providerCoverageDashboardSummary.userFacingSummary && card.providerCoverageDashboardSummary.userFacingSummary.resultLabel || 'Provider 覆盖仍需复核') + '</p><p>' + escapeHtml(card.readOnlyProviderSandboxIntegrationGateSummary && card.readOnlyProviderSandboxIntegrationGateSummary.userFacingSummary && card.readOnlyProviderSandboxIntegrationGateSummary.userFacingSummary.resultLabel || '只读 Provider Sandbox 接入闸门仍需复核') + '</p><p>' + escapeHtml(card.sandboxPriceCandidateSessionSummary && card.sandboxPriceCandidateSessionSummary.userFacingSummary && card.sandboxPriceCandidateSessionSummary.userFacingSummary.resultLabel || 'Sandbox 价格候选会话仍需复核') + '</p><p>' + escapeHtml(card.sandboxPriceCandidateResultBoardSummary && card.sandboxPriceCandidateResultBoardSummary.title || 'Sandbox 价格候选结果') + '</p><p>覆盖来源不代表全网覆盖</p><p>bookingUrl:null</p><p>payment:false</p><p>order:false</p></div><div data-commerce-global-shopping-source-trust-output="true"><p>只读来源可信度评分</p><p>' + escapeHtml(card.readOnlySourceTrustScoreSummary && card.readOnlySourceTrustScoreSummary.userFacingSummary && card.readOnlySourceTrustScoreSummary.userFacingSummary.resultLabel || '来源可信度仍需复核') + '</p><p>' + escapeHtml(card.sandboxPriceCandidateResultBoardSummary && card.sandboxPriceCandidateResultBoardSummary.caveat || '当前仅展示只读 sandbox 候选结果，不代表真实价格、全网最低、最低价保证、锁价、可订、付款、下单或出票能力。') + '</p><p>可信度不代表官方背书</p><p>低价不等于最佳</p><p>bookingUrl:null</p><p>payment:false</p><p>order:false</p></div></section>' : '';
    const sandboxDecisionReviewHtml = card.sandboxDecisionReviewViewModelSummary ? '<section class="commerce-global-shopping-sandbox-decision-review" data-commerce-global-shopping-sandbox-decision-review="true"><h5>Sandbox 候选决策复核</h5><p>Sandbox 候选对比工作台</p><p>Provider 证据对比矩阵</p><p>只读跳转交接演练</p><p>候选对比</p><p>证据矩阵</p><p>交接演练</p><p>Sandbox 候选对比已准备</p><p>Provider 证据矩阵已准备</p><p>只读交接演练已准备</p><p>候选推荐不代表最低价保证</p><p>交接演练不打开平台</p><p>参数预览不包含身份或支付信息</p><p>决策复核不代表下单能力</p><p>当前仅用于复核 sandbox 候选</p><p>不代表真实价格、全网最低、锁价、可订、付款、下单或出票能力</p><button type="button" class="cmd-btn gray" data-commerce-global-shopping-sandbox-comparison-show="true">查看候选对比</button><button type="button" class="cmd-btn gray" data-commerce-global-shopping-evidence-matrix-show="true">查看证据矩阵</button><button type="button" class="cmd-btn gray" data-commerce-global-shopping-handoff-drill-show="true">查看交接演练</button><div data-commerce-global-shopping-sandbox-comparison-output="true"><p>Sandbox 候选对比工作台</p><p>' + escapeHtml(card.sandboxCandidateComparisonWorkbenchSummary && card.sandboxCandidateComparisonWorkbenchSummary.userFacingSummary && card.sandboxCandidateComparisonWorkbenchSummary.userFacingSummary.resultLabel || '候选对比仍需复核') + '</p><p>候选推荐不代表最低价保证</p><p>bookingUrl:null</p><p>payment:false</p><p>order:false</p></div><div data-commerce-global-shopping-evidence-matrix-output="true"><p>Provider 证据对比矩阵</p><p>' + escapeHtml(card.providerEvidenceComparisonMatrixSummary && card.providerEvidenceComparisonMatrixSummary.userFacingSummary && card.providerEvidenceComparisonMatrixSummary.userFacingSummary.resultLabel || '证据矩阵仍需复核') + '</p><p>当前仅展示脱敏证据摘要</p><p>bookingUrl:null</p><p>payment:false</p><p>order:false</p></div><div data-commerce-global-shopping-handoff-drill-output="true"><p>只读跳转交接演练</p><p>' + escapeHtml(card.readOnlyHandoffReadinessDrillSummary && card.readOnlyHandoffReadinessDrillSummary.userFacingSummary && card.readOnlyHandoffReadinessDrillSummary.userFacingSummary.resultLabel || '交接演练仍需复核') + '</p><p>交接演练不打开平台</p><p>参数预览不包含身份或支付信息</p><p>bookingUrl:null</p><p>payment:false</p><p>order:false</p></div></section>' : '';
