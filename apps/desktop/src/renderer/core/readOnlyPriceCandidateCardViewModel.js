;(function () {
  ...
  const READ_ONLY_PRICE_CANDIDATE_CARD_VIEW_MODEL_VERSION = "2.2.6";
  ...
  function renderReadOnlyPriceCandidateCardHtml(input) {
    ...
    const manualPlatformReviewHtml = card.userManualReviewViewModelSummary ? '<section class="commerce-global-shopping-manual-platform-review" data-commerce-global-shopping-manual-platform-review="true"><h5>用户手动复核与安全下一步</h5><p>用户手动复核与安全下一步</p><p>用户手动复核流程</p><p>平台核对进度追踪</p><p>安全下一步</p><p>手动复核流程</p><p>平台核对进度</p><p>风险说明</p><p>用户手动复核流程已准备</p><p>平台核对进度已准备</p><p>安全下一步已准备</p><p>平台核对进度不保存勾选</p><p>安全下一步不打开平台</p><p>下一步不包含购买、下单、付款或出票</p><p>用户必须自行完成最终平台判断</p><p>当前只展示手动复核流程、平台核对进度和安全下一步</p><p>不打开平台，不保存选择，不构成订单、付款授权或签名</p><button type="button" class="cmd-btn gray" data-commerce-global-shopping-user-facing-manual-review-flow-show="true">查看手动复核流程</button><button type="button" class="cmd-btn gray" data-commerce-global-shopping-platform-verification-progress-show="true">查看平台核对进度</button><button type="button" class="cmd-btn gray" data-commerce-global-shopping-safe-next-action-show="true">查看安全下一步</button><div data-commerce-global-shopping-user-facing-manual-review-flow-output="true"><p>用户手动复核流程</p><p>' + escapeHtml(card.userFacingManualReviewFlowSummary && card.userFacingManualReviewFlowSummary.userFacingSummary && card.userFacingManualReviewFlowSummary.userFacingSummary.resultLabel || '用户手动复核流程仍需复核') + '</p><p>当前只展示手动复核流程、平台核对进度和安全下一步</p></div><div data-commerce-global-shopping-platform-verification-progress-output="true"><p>平台核对进度追踪</p><p>' + escapeHtml(card.platformVerificationProgressTrackerSummary && card.platformVerificationProgressTrackerSummary.userFacingSummary && card.platformVerificationProgressTrackerSummary.userFacingSummary.resultLabel || '平台核对进度仍需复核') + '</p><p>平台核对进度不保存勾选</p></div><div data-commerce-global-shopping-safe-next-action-output="true"><p>安全下一步</p><p>' + escapeHtml(card.safeNextActionPanelSummary && card.safeNextActionPanelSummary.userFacingSummary && card.safeNextActionPanelSummary.userFacingSummary.resultLabel || '安全下一步仍需复核') + '</p><p>安全下一步不打开平台</p><p>下一步不包含购买、下单、付款或出票</p><p>用户必须自行完成最终平台判断</p></div></section>' : '';
