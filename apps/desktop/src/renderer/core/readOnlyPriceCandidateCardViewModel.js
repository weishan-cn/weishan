;(function () {
  ...
  const READ_ONLY_PRICE_CANDIDATE_CARD_VIEW_MODEL_VERSION = "4.0.0";
  ...
  function renderReadOnlyPriceCandidateCardHtml(input) {
    ...
    const globalShoppingPublicBetaReviewHtml = card.globalShoppingPublicBetaViewModelSummary ? '<section class="commerce-global-shopping-public-beta-review" data-commerce-global-shopping-public-beta-review="true"><h5>Global Shopping Public Beta Review</h5><p>Global Shopping Read-Only Public Beta Shell</p><p>Provider-Zero Runtime Lock</p><p>User Trust Launch Board</p><p>Public Beta Safety Copy Center</p><p>Public Beta</p><p>Provider-Zero Lock</p><p>User Trust Launch</p><p>Safety Copy</p><p>' + escapeHtml(card.globalShoppingReadOnlyPublicBetaShellSummary && card.globalShoppingReadOnlyPublicBetaShellSummary.userFacingSummary && card.globalShoppingReadOnlyPublicBetaShellSummary.userFacingSummary.resultLabel || 'Global Shopping Read-Only Public Beta Shell 仍需复核') + '</p><p>' + escapeHtml(card.providerZeroRuntimeLockSummary && card.providerZeroRuntimeLockSummary.userFacingSummary && card.providerZeroRuntimeLockSummary.userFacingSummary.resultLabel || 'Provider-Zero Runtime Lock 仍需复核') + '</p><p>' + escapeHtml(card.userTrustLaunchBoardSummary && card.userTrustLaunchBoardSummary.userFacingSummary && card.userTrustLaunchBoardSummary.userFacingSummary.resultLabel || 'User Trust Launch Board 仍需复核') + '</p><p>' + escapeHtml(card.publicBetaSafetyCopyCenterSummary && card.publicBetaSafetyCopyCenterSummary.userFacingSummary && card.publicBetaSafetyCopyCenterSummary.userFacingSummary.resultLabel || 'Public Beta Safety Copy Center 仍需复核') + '</p><p>Global Shopping Read-Only Public Beta Shell 已准备</p><p>Provider-Zero Runtime Lock 已准备</p><p>User Trust Launch Board 已准备</p><p>Public Beta Safety Copy Center 已准备</p><p>Public Beta 只提供候选价证据，不付款、不下单、不出票</p><p>Provider-Zero Lock 不接真实 provider、不读密钥、不联网</p><p>User Trust Launch 不执行真实 launch</p><p>Safety Copy 不承诺最低价、最终价或官方背书</p><p>Human public beta review 仍需人工复核</p><p>当前只展示 Global Shopping Public Beta Review</p><p>不接真实 provider，不读取密钥，不联网，不打开平台，不创建 release，不 push，不付款、不下单、不出票</p><p>当前已覆盖来源中的较低候选价</p><p>与官方价对比</p><p>价格以跳转后平台实时页面为准</p><p>当前仅提供只读候选证据，不提供付款、下单或出票能力</p><button type="button" class="cmd-btn gray" data-commerce-global-shopping-public-beta-show="true">查看 Public Beta</button><button type="button" class="cmd-btn gray" data-commerce-global-shopping-provider-zero-lock-show="true">查看 Provider-Zero Lock</button><button type="button" class="cmd-btn gray" data-commerce-global-shopping-user-trust-launch-show="true">查看 User Trust Launch</button><button type="button" class="cmd-btn gray" data-commerce-global-shopping-safety-copy-show="true">查看 Safety Copy</button><div data-commerce-global-shopping-public-beta-output="true"><p>Global Shopping Read-Only Public Beta Shell</p><p>' + escapeHtml(card.globalShoppingReadOnlyPublicBetaShellSummary && card.globalShoppingReadOnlyPublicBetaShellSummary.userFacingSummary && card.globalShoppingReadOnlyPublicBetaShellSummary.userFacingSummary.resultLabel || 'Global Shopping Read-Only Public Beta Shell 仍需复核') + '</p><p>Public Beta 只提供候选价证据，不付款、不下单、不出票</p><p>当前已覆盖来源中的较低候选价</p><p>与官方价对比</p></div><div data-commerce-global-shopping-provider-zero-lock-output="true"><p>Provider-Zero Runtime Lock</p><p>' + escapeHtml(card.providerZeroRuntimeLockSummary && card.providerZeroRuntimeLockSummary.userFacingSummary && card.providerZeroRuntimeLockSummary.userFacingSummary.resultLabel || 'Provider-Zero Runtime Lock 仍需复核') + '</p><p>Provider-Zero Lock 不接真实 provider、不读密钥、不联网</p></div><div data-commerce-global-shopping-user-trust-launch-output="true"><p>User Trust Launch Board</p><p>' + escapeHtml(card.userTrustLaunchBoardSummary && card.userTrustLaunchBoardSummary.userFacingSummary && card.userTrustLaunchBoardSummary.userFacingSummary.resultLabel || 'User Trust Launch Board 仍需复核') + '</p><p>User Trust Launch 不执行真实 launch</p><p>Weishan 可尽量带入搜索条件，但用户需在对应平台自行确认价格、登录、填写资料并完成下单</p></div><div data-commerce-global-shopping-safety-copy-output="true"><p>Public Beta Safety Copy Center</p><p>' + escapeHtml(card.publicBetaSafetyCopyCenterSummary && card.publicBetaSafetyCopyCenterSummary.userFacingSummary && card.publicBetaSafetyCopyCenterSummary.userFacingSummary.resultLabel || 'Public Beta Safety Copy Center 仍需复核') + '</p><p>Safety Copy 不承诺最低价、最终价或官方背书</p><p>价格以跳转后平台实时页面为准</p><p>当前仅提供只读候选证据，不提供付款、下单或出票能力</p></div></section>' : '';
