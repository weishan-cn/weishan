;(function () {
  ...
  const FLIGHT_WORKFLOW_SAFETY_REGRESSION_SENTINEL_VERSION = "2.1.84";
  ...
  function buildFlightWorkflowSafetyRegressionReport(input) {
    try {
      ...
      return sanitizeReport({ sentinelName:SENTINEL_NAME, appVersion:FLIGHT_WORKFLOW_SAFETY_REGRESSION_SENTINEL_VERSION, status:status, checks:checks, failures:failures, warnings:scan.warnings || [], pilotExitCriteriaSummary:pilotExitCriteriaSummary, launchCandidateReadinessSummary:launchCandidateReadinessSummary, freezeGateSummary:freezeGateSummary, evidenceFreezePackSummary:evidenceFreezePackSummary, safety:safety(), redacted:true });
    } catch (error) {
