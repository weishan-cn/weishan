(function(){
  const COMPACT_FLIGHT_RESULT_CARD_V1_VERSION = "2.1.43";
  ...
  function buildCompactFlightResultCard(input){
    ...
    const badgesResult = badgesApi && typeof badgesApi.formatResultBadges === "function"
      ? badgesApi.formatResultBadges(safe.badges || ["Limited Beta", "只读价格", "不可下单", "最终以平台页面为准"])
      : { badges:safe.badges || ["Limited Beta", "只读价格", "不可下单", "以平台页面为准"], displayText:"Limited Beta｜只读价格｜不可下单｜以平台页面为准", badgeSeparated:true };
    ...
    const card = {
      ...
      priceTruthText:text(safe.priceTruthText || "Limited Beta 只读验证价，不代表真实最低价"),
  ...
  function assertCompactFlightResultCardSafe(card){
    ...
    if (!Array.isArray(value.badges) || String(value.badgeDisplayText || "").indexOf("Limited Beta只读价格不可下单") >= 0) throw new Error("badges must not concatenate");
