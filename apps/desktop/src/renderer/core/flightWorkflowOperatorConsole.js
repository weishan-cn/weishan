;(function () {
  "use strict";
  const FLIGHT_WORKFLOW_OPERATOR_CONSOLE_VERSION = "2.1.67";
  ...
  function buildFlightWorkflowOperatorConsole(input) {
    try {
      if (!input || typeof input !== "object" || Array.isArray(input)) return sanitizeFlightWorkflowOperatorConsole({ consoleName:CONSOLE_NAME, appVersion:FLIGHT_WORKFLOW_OPERATOR_CONSOLE_VERSION, status:"failed_safe", workflowId:"deterministic-flight-workflow-operator-console-v2.1.67", readiness:evaluateFlightWorkflowOperatorReadiness(null), sections:[], userFacingSummary:{ title:"机票工作流运营控制台", resultLabel:"已安全阻断", caveat:"唯珊只提供只读候选证据，不付款、不下单、不出票。", redacted:true }, nextOperatorAction:{ actionId:"failed_safe", label:"安全降级", enabled:false, requiresUserConfirmation:false, redacted:true }, safety:safety(), redacted:true });
