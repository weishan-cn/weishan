;(function () {
  ...
  const READ_ONLY_QUOTE_EVIDENCE_SUMMARY_FORMATTER_VERSION = "4.1.1";
  ...
  function formatFlightWorkflowSummary(input) {
    ...
    return clone({
      ...
      userFacingManualReviewFlowStatus: safeLine(safe.userFacingManualReviewFlowStatus || ""),
      platformVerificationProgressStatus: safeLine(safe.platformVerificationProgressStatus || ""),
      safeNextActionPanelStatus: safeLine(safe.safeNextActionPanelStatus || ""),
      userManualReviewViewModelStatus: safeLine(safe.userManualReviewViewModelStatus || ""),
      ...
      providerLegalReviewStatus: safeLine(safe.providerLegalReviewStatus || ""),
      credentialVaultInterfaceStatus: safeLine(safe.credentialVaultInterfaceStatus || ""),
      sandboxAdapterContractStatus: safeLine(safe.sandboxAdapterContractStatus || ""),
      providerIntegrationPrepViewModelStatus: safeLine(safe.providerIntegrationPrepViewModelStatus || ""),
      sandboxProviderMockRuntimeStatus: safeLine(safe.sandboxProviderMockRuntimeStatus || ""),
      vaultBoundaryContractStatus: safeLine(safe.vaultBoundaryContractStatus || ""),
      legalApprovalWorkflowStatus: safeLine(safe.legalApprovalWorkflowStatus || ""),
      providerMockRuntimeViewModelStatus: safeLine(safe.providerMockRuntimeViewModelStatus || ""),
      mockProviderAdapterRegistryStatus: safeLine(safe.mockProviderAdapterRegistryStatus || ""),
      providerContractReplayStatus: safeLine(safe.providerContractReplayStatus || ""),
      providerLaunchReadinessStatus: safeLine(safe.providerLaunchReadinessStatus || ""),
      providerLaunchReadinessViewModelStatus: safeLine(safe.providerLaunchReadinessViewModelStatus || ""),
      safeToProceedWithProviderSandboxContractImplementation: safe.safeToProceedWithProviderSandboxContractImplementation === true,
      safeToProceedWithMockAdapterRuntimeHardening: safe.safeToProceedWithMockAdapterRuntimeHardening === true,
      safeToProceedWithHumanProviderSandboxApproval: safe.safeToProceedWithHumanProviderSandboxApproval === true,
  ...
  function formatProviderFinalSafetySealSummary(input) {
    const safe = input && typeof input === "object" ? input : {};
    const summary = safe.providerFinalSafetySealSummary || safe.summary || (safe.title || safe.status ? safe : {});
    return clone({ title:"Provider Final Safety Seal", line:safeLine(obj(summary.userFacingSummary).resultLabel || "Provider Final Safety Seal 仍需复核"), sectionLabels:["Safety Seal", "Final Review", "Evidence Summary", "Decision Matrix"], status:text(summary.status || "needs_review"), caveat:safeLine(obj(summary.userFacingSummary).caveat || "Safety Seal 不生成真实证书、不写文件。"), bookingUrl:null, checkoutUrl:null, paymentUrl:null, orderUrl:null, redacted:true });
  }

  function formatOfflineActivationWarRoomSummary(input) {
    const safe = input && typeof input === "object" ? input : {};
    const summary = safe.offlineActivationWarRoomSummary || safe.summary || (safe.title || safe.status ? safe : {});
    return clone({ title:"Offline Activation War Room", line:safeLine(obj(summary.userFacingSummary).resultLabel || "Offline Activation War Room 仍需复核"), sectionLabels:["Activation War Room", "Blockers", "Boundary Verifier", "Next Human Review"], status:text(summary.status || "needs_review"), caveat:safeLine(obj(summary.userFacingSummary).caveat || "Activation War Room 不激活 sandbox、不启用 provider。"), bookingUrl:null, checkoutUrl:null, paymentUrl:null, orderUrl:null, redacted:true });
  }

  function formatReadOnlyProviderReadinessCertificateSummary(input) {
    const safe = input && typeof input === "object" ? input : {};
    const summary = safe.readOnlyProviderReadinessCertificateSummary || safe.summary || (safe.title || safe.status ? safe : {});
    return clone({ title:"Read-Only Provider Readiness Certificate", line:safeLine(obj(summary.userFacingSummary).resultLabel || "Read-Only Provider Readiness Certificate 仍需复核"), sectionLabels:["Readiness Certificate", "Evidence Summary", "Final Dossier", "Verify Chain"], status:text(summary.status || "needs_review"), caveat:safeLine(obj(summary.userFacingSummary).caveat || "Readiness Certificate 不持久化证书。"), bookingUrl:null, checkoutUrl:null, paymentUrl:null, orderUrl:null, redacted:true });
  }

  function formatProviderNoActivationGuaranteeBoardSummary(input) {
    const safe = input && typeof input === "object" ? input : {};
    const summary = safe.providerNoActivationGuaranteeBoardSummary || safe.summary || (safe.title || safe.status ? safe : {});
    return clone({ title:"Provider No-Activation Guarantee Board", line:safeLine(obj(summary.userFacingSummary).resultLabel || "Provider No-Activation Guarantee Board 仍需复核"), sectionLabels:["No-Activation Guarantee", "Safety Seal", "War Room", "Regression Guard"], status:text(summary.status || "needs_review"), caveat:safeLine(obj(summary.userFacingSummary).caveat || "No-Activation Guarantee 不修改配置、不执行真实阻断。"), bookingUrl:null, checkoutUrl:null, paymentUrl:null, orderUrl:null, redacted:true });
  }

  function formatProviderFinalSafetyViewModelSummary(input) {
    const safe = input && typeof input === "object" ? input : {};
    const summary = safe.providerFinalSafetyViewModelSummary || safe.viewModelSummary || (safe.title || safe.status ? safe : {});
  ...
  function formatProviderReadOnlyPublicReleaseCenterSummary(input) {
    const safe = input && typeof input === "object" ? input : {};
    const summary = safe.providerReadOnlyPublicReleaseCenterSummary || safe.summary || (safe.title || safe.status ? safe : {});
    return clone({ title:"Provider Read-Only Public Release Center", line:safeLine(obj(summary.userFacingSummary).resultLabel || "Provider Read-Only Public Release Center 仍需复核"), sectionLabels:["Public Release", "Export Preview", "No-Provider Receipt", "Safety Statement"], status:text(summary.status || "needs_review"), caveat:safeLine(obj(summary.userFacingSummary).caveat || "Public Release 不创建真实公开发布。"), bookingUrl:null, checkoutUrl:null, paymentUrl:null, orderUrl:null, redacted:true });
  }

  function formatTrustClosureExportPreviewSummary(input) {
    const safe = input && typeof input === "object" ? input : {};
    const summary = safe.trustClosureExportPreviewSummary || safe.summary || (safe.title || safe.status ? safe : {});
    return clone({ title:"Trust Closure Export Preview", line:safeLine(obj(summary.userFacingSummary).resultLabel || "Trust Closure Export Preview 仍需复核"), sectionLabels:["Public Release", "Export Preview", "No-Provider Receipt", "Safety Statement"], status:text(summary.status || "needs_review"), caveat:safeLine(obj(summary.userFacingSummary).caveat || "Export Preview 不生成真实导出文件。"), bookingUrl:null, checkoutUrl:null, paymentUrl:null, orderUrl:null, redacted:true });
  }

  function formatFinalNoProviderBoundaryReceiptSummary(input) {
    const safe = input && typeof input === "object" ? input : {};
    const summary = safe.finalNoProviderBoundaryReceiptSummary || safe.summary || (safe.title || safe.status ? safe : {});
    return clone({ title:"Final No-Provider Boundary Receipt", line:safeLine(obj(summary.userFacingSummary).resultLabel || "Final No-Provider Boundary Receipt 仍需复核"), sectionLabels:["Public Release", "Export Preview", "No-Provider Receipt", "Safety Statement"], status:text(summary.status || "needs_review"), caveat:safeLine(obj(summary.userFacingSummary).caveat || "No-Provider Receipt 不生成真实回执、不打开平台。"), bookingUrl:null, checkoutUrl:null, paymentUrl:null, orderUrl:null, redacted:true });
  }

  function formatPublicSafetyStatementPreviewSummary(input) {
    const safe = input && typeof input === "object" ? input : {};
    const summary = safe.publicSafetyStatementPreviewSummary || safe.summary || (safe.title || safe.status ? safe : {});
    return clone({ title:"Public Safety Statement Preview", line:safeLine(obj(summary.userFacingSummary).resultLabel || "Public Safety Statement Preview 仍需复核"), sectionLabels:["Public Release", "Export Preview", "No-Provider Receipt", "Safety Statement"], status:text(summary.status || "needs_review"), caveat:safeLine(obj(summary.userFacingSummary).caveat || "Safety Statement 不承诺最低价、最终价或官方背书。"), bookingUrl:null, checkoutUrl:null, paymentUrl:null, orderUrl:null, redacted:true });
  }

  function formatProviderPublicReleaseViewModelSummary(input) {
    const safe = input && typeof input === "object" ? input : {};
    const summary = safe.providerPublicReleaseViewModelSummary || safe.viewModelSummary || (safe.title || safe.status ? safe : {});
  ...
  window.WeishanReadOnlyQuoteEvidenceSummaryFormatter = {
    ...
    formatProviderFinalSafetySealSummary,
    formatOfflineActivationWarRoomSummary,
    ...
    formatProviderReadOnlyPublicReleaseCenterSummary,
    formatTrustClosureExportPreviewSummary,
