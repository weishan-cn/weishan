;(function () {
  ...
  const SECURE_KEY_STORAGE_PLAN_VERSION = "2.1.0";
  ...
  function defaultDisplay() {
    return {
      ...
      secureStorageDesignGateNextStepLine: "下一步：只读 provider sandbox gate",
  ...
  function buildSecureKeyStoragePlanReadinessSummary(plan) {
    ...
    return clone({
      ...
      secureStorageDesignGateNextStepLine: display.secureStorageDesignGateNextStepLine || "下一步：只读 provider sandbox gate",
      ...
      nextStepText: display.nextStepText || "provider endpoint allowlist 闸门：已建立。下一步：只读 provider sandbox gate。key 删除 / 轮换 / 过期机制草案已建立，但当前版本仍不能输入、保存、读取、删除、轮换或测试真实 API key。",
