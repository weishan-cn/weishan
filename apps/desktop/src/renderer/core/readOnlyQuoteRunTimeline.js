;(function () {
  ...
  const READ_ONLY_QUOTE_RUN_TIMELINE_VERSION = "2.1.60";
  ...
  function summarizeReadOnlyQuoteRunTimeline(timeline) {
    ...
    return clone({
      ...
      runId: text(safe.runId || "deterministic-v2.1.60-read-only-sandbox-run"),
  ...
  function buildReadOnlyQuoteRunTimeline(runResult, options) {
    ...
    const summary = summarizeReadOnlyQuoteRunTimeline({
      runId: text(safe.runId || safeOptions.runId || "deterministic-v2.1.60-read-only-sandbox-run"),
