;(function () {
  ...
  const FLIGHT_WORKFLOW_RISK_BADGE_BUILDER_VERSION = "4.2.8";
  ...
  function buildFlightWorkflowRiskBadges(input) {
    try {
      ...
      const publicBetaCandidateEvidenceReviewSummary = obj(safe.publicBetaCandidateEvidenceReviewSummary);
      ...
      if (publicBetaCandidateEvidenceReviewSummary.status === "manual_review_required") badges.push(badge("public_beta_candidate_evidence_review_manual", "候选证据需人工复核", "warning"));
      if (publicBetaCandidateEvidenceReviewSummary.status === "needs_review") badges.push(badge("public_beta_candidate_evidence_review_review", "候选证据仍需复核", "warning"));
      if (publicBetaCandidateEvidenceReviewSummary.status === "blocked") badges.push(badge("public_beta_candidate_evidence_review_blocked", "候选证据已阻断", "blocked"));
      if (trialOperatorNotesPanelSummary.status === "manual_review_required") badges.push(badge("trial_operator_notes_panel_manual", "运营备注需人工复核", "warning"));
      if (trialOperatorNotesPanelSummary.status === "needs_review") badges.push(badge("trial_operator_notes_panel_review", "运营备注仍需复核", "warning"));
      if (trialOperatorNotesPanelSummary.status === "blocked") badges.push(badge("trial_operator_notes_panel_blocked", "运营备注已阻断", "blocked"));
      if (offlineSafetyDeltaBoardSummary.status === "manual_review_required") badges.push(badge("offline_safety_delta_board_manual", "安全边界差异需人工复核", "warning"));
      if (offlineSafetyDeltaBoardSummary.status === "needs_review") badges.push(badge("offline_safety_delta_board_review", "安全边界差异仍需复核", "warning"));
      if (offlineSafetyDeltaBoardSummary.status === "blocked") badges.push(badge("offline_safety_delta_board_blocked", "安全边界差异已阻断", "blocked"));
      ...
      if (obj(safe.publicBetaFreezeEvidenceSummary).status || obj(safe.publicBetaFreezeEvidenceSummary).freezeEvidenceStatus) badges.push(badge("public_beta_freeze_evidence_summary_ready", labelOf(obj(safe.publicBetaFreezeEvidenceSummary), "Public Beta Freeze Evidence Summary 仍需复核"), obj(safe.publicBetaFreezeEvidenceSummary).status === "blocked" ? "blocked" : (obj(safe.publicBetaFreezeEvidenceSummary).status === "ready" ? "info" : "warning")));
      if (obj(safe.manualTrialIssueReviewBoardSummary).status || obj(safe.manualTrialIssueReviewBoardSummary).issueReviewStatus) badges.push(badge("manual_trial_issue_review_board_ready", labelOf(obj(safe.manualTrialIssueReviewBoardSummary), "Manual Trial Issue Review Board 仍需复核"), obj(safe.manualTrialIssueReviewBoardSummary).status === "blocked" ? "blocked" : (obj(safe.manualTrialIssueReviewBoardSummary).status === "ready" ? "info" : "warning")));
      if (obj(safe.offlineAcceptanceSnapshotSummary).status || obj(safe.offlineAcceptanceSnapshotSummary).acceptanceSnapshotStatus) badges.push(badge("offline_acceptance_snapshot_ready", labelOf(obj(safe.offlineAcceptanceSnapshotSummary), "Offline Acceptance Snapshot 仍需复核"), obj(safe.offlineAcceptanceSnapshotSummary).status === "blocked" ? "blocked" : "warning"));
      if (obj(safe.publicBetaAcceptanceSnapshotViewModelSummary).status) badges.push(badge("public_beta_acceptance_snapshot_view_model_ready", labelOf(obj(safe.publicBetaAcceptanceSnapshotViewModelSummary), "Public Beta Acceptance Snapshot View Model 仍需复核"), obj(safe.publicBetaAcceptanceSnapshotViewModelSummary).status === "blocked" ? "blocked" : (obj(safe.publicBetaAcceptanceSnapshotViewModelSummary).status === "ready" ? "info" : "warning")));
      if (obj(safe.publicBetaFreezeEvidenceSummary).status || obj(safe.publicBetaAcceptanceSnapshotViewModelSummary).status) badges.push(badge("public_beta_freeze_evidence_scope", "Freeze Evidence / 冻结证据仅为只读摘要，不修改配置", "warning"));
      if (obj(safe.manualTrialIssueReviewBoardSummary).status || obj(safe.publicBetaAcceptanceSnapshotViewModelSummary).status) badges.push(badge("manual_trial_issue_review_boundary", "Issue Review / 问题复核仅为离线视图，不创建真实 issue", "warning"));
      if (obj(safe.offlineAcceptanceSnapshotSummary).status || obj(safe.publicBetaAcceptanceSnapshotViewModelSummary).status) badges.push(badge("offline_acceptance_snapshot_boundary", "Acceptance Snapshot / 验收快照不写文件、不导出", "warning"));
      if ((obj(safe.publicBetaAcceptanceSnapshotViewModelSummary).status && obj(safe.publicBetaAcceptanceSnapshotViewModelSummary).status !== "blocked") || safe.safeToProceedWithManualAcceptanceSnapshotReview === true) badges.push(badge("public_beta_acceptance_snapshot_manual_review_required", "仍需人工复核后再决定下一阶段", "warning"));
      if (obj(safe.publicBetaAcceptanceReviewConsoleSummary).status || obj(safe.publicBetaAcceptanceReviewConsoleSummary).acceptanceReviewStatus) badges.push(badge("public_beta_acceptance_review_console_ready", labelOf(obj(safe.publicBetaAcceptanceReviewConsoleSummary), "Public Beta Acceptance Review Console 仍需复核"), obj(safe.publicBetaAcceptanceReviewConsoleSummary).status === "blocked" ? "blocked" : "warning"));
      if (obj(safe.offlineTrialClosureBoardSummary).status || obj(safe.offlineTrialClosureBoardSummary).closureStatus) badges.push(badge("offline_trial_closure_board_ready", labelOf(obj(safe.offlineTrialClosureBoardSummary), "Offline Trial Closure Board 仍需复核"), obj(safe.offlineTrialClosureBoardSummary).status === "blocked" ? "blocked" : "warning"));
      if (obj(safe.noLaunchAssuranceGateSummary).status) badges.push(badge("no_launch_assurance_gate_ready", labelOf(obj(safe.noLaunchAssuranceGateSummary), "No-Launch Assurance Gate 仍需复核"), obj(safe.noLaunchAssuranceGateSummary).status === "blocked" ? "blocked" : (obj(safe.noLaunchAssuranceGateSummary).status === "ready" ? "info" : "warning")));
      if (obj(safe.publicBetaClosureReviewViewModelSummary).status) badges.push(badge("public_beta_closure_review_view_model_ready", labelOf(obj(safe.publicBetaClosureReviewViewModelSummary), "Public Beta Closure Review View Model 仍需复核"), obj(safe.publicBetaClosureReviewViewModelSummary).status === "blocked" ? "blocked" : (obj(safe.publicBetaClosureReviewViewModelSummary).status === "ready" ? "info" : "warning")));
      if (obj(safe.publicBetaAcceptanceReviewConsoleSummary).status || obj(safe.publicBetaClosureReviewViewModelSummary).status) badges.push(badge("public_beta_acceptance_review_boundary", "当前不发布、不创建 release、不 push", "warning"));
      if (obj(safe.offlineTrialClosureBoardSummary).status || obj(safe.publicBetaClosureReviewViewModelSummary).status) badges.push(badge("offline_trial_closure_boundary", "试用闭环仅为离线视图，不关闭真实任务", "warning"));
      if (obj(safe.noLaunchAssuranceGateSummary).status || obj(safe.publicBetaClosureReviewViewModelSummary).status) badges.push(badge("no_launch_assurance_boundary", "仍不允许启用 provider、付款、下单或发布", "warning"));
      if ((obj(safe.publicBetaClosureReviewViewModelSummary).status && obj(safe.publicBetaClosureReviewViewModelSummary).status !== "blocked") || safe.safeToProceedWithManualClosureReview === true) badges.push(badge("public_beta_closure_review_manual_review_required", "验收复核后仍需人工决定下一阶段", "warning"));
