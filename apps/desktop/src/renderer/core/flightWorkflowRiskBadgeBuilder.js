;(function () {
  ...
  const FLIGHT_WORKFLOW_RISK_BADGE_BUILDER_VERSION = "4.1.5";
  ...
  function buildFlightWorkflowRiskBadges(input) {
    try {
      ...
      if (obj(safe.publicBetaQaFreezeGateSummary).status || obj(safe.publicBetaQaFreezeGateSummary).freezeStatus) badges.push(badge("public_beta_qa_freeze_gate_ready", labelOf(obj(safe.publicBetaQaFreezeGateSummary), "Public Beta QA Freeze Gate 仍需复核"), obj(safe.publicBetaQaFreezeGateSummary).status === "blocked" ? "blocked" : (obj(safe.publicBetaQaFreezeGateSummary).status === "ready" ? "info" : "warning")));
      if (obj(safe.manualTrialSummaryBoardSummary).status || obj(safe.manualTrialSummaryBoardSummary).trialSummaryStatus) badges.push(badge("manual_trial_summary_board_ready", labelOf(obj(safe.manualTrialSummaryBoardSummary), "Manual Trial Summary Board 仍需复核"), obj(safe.manualTrialSummaryBoardSummary).status === "blocked" ? "blocked" : (obj(safe.manualTrialSummaryBoardSummary).status === "ready" ? "info" : "warning")));
      if (obj(safe.offlineReadinessReviewPanelSummary).status || obj(safe.offlineReadinessReviewPanelSummary).readinessStatus) badges.push(badge("offline_readiness_review_panel_ready", labelOf(obj(safe.offlineReadinessReviewPanelSummary), "Offline Readiness Review Panel 仍需复核"), obj(safe.offlineReadinessReviewPanelSummary).status === "blocked" ? "blocked" : "warning"));
      if (obj(safe.publicBetaFreezeReviewViewModelSummary).status) badges.push(badge("public_beta_freeze_review_ready", labelOf(obj(safe.publicBetaFreezeReviewViewModelSummary), "Public Beta Freeze Review View Model 仍需复核"), obj(safe.publicBetaFreezeReviewViewModelSummary).status === "blocked" ? "blocked" : (obj(safe.publicBetaFreezeReviewViewModelSummary).status === "ready" ? "info" : "warning")));
      if (obj(safe.publicBetaQaFreezeGateSummary).status || obj(safe.manualTrialSummaryBoardSummary).status || obj(safe.offlineReadinessReviewPanelSummary).status) badges.push(badge("public_beta_freeze_allowed_actions", "Allowed Next Actions / 继续测试 / 人工复核 / 阻断", "warning"));
      if (obj(safe.publicBetaFreezeReviewViewModelSummary).status || obj(safe.offlineReadinessReviewPanelSummary).status) badges.push(badge("public_beta_freeze_blocked_actions", "Blocked Next Actions / 启用 provider / 付款 / 下单 / 发布", "warning"));
      if (obj(safe.publicBetaQaFreezeGateSummary).status || obj(safe.publicBetaFreezeReviewViewModelSummary).status) badges.push(badge("public_beta_freeze_scope", "当前冻结的是只读 QA 范围，不执行真实 freeze", "warning"));
      if ((obj(safe.publicBetaFreezeReviewViewModelSummary).status && obj(safe.publicBetaFreezeReviewViewModelSummary).status !== "blocked") || safe.safeToProceedWithManualFreezeReview === true) badges.push(badge("public_beta_freeze_manual_review_required", "人工复核后再决定下一阶段", "warning"));
