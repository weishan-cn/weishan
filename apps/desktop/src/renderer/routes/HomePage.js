(function(){
  ...
  function globalProcurementPlanHtml(task, options){
    ...
    const detail = globalProcurementDetailQuality(task);
    if (!plan || !detail) return "";
    const opts = options && typeof options === "object" ? options : {};
    if (opts.hideBlockedInSummary && plan.status === "blocked") return "";
    const identifiedConditions = Array.isArray(detail.identifiedConditions) && detail.identifiedConditions.length ? detail.identifiedConditions : ["当前条件待补充"];
    const missingInfoList = Array.isArray(detail.missingInfoList) && detail.missingInfoList.length ? detail.missingInfoList : ["当前无关键缺口"];
    const safeGuidanceList = Array.isArray(detail.safeGuidanceList) && detail.safeGuidanceList.length ? detail.safeGuidanceList : ["保持人工确认，不自动执行采购动作"];
    const policyLines = Array.isArray(detail.externalSearchPolicyLines) && detail.externalSearchPolicyLines.length ? detail.externalSearchPolicyLines : ["当前不显示外部搜索入口"];
    const safetyBoundaryList = Array.isArray(detail.safetyBoundaryList) && detail.safetyBoundaryList.length ? detail.safetyBoundaryList : ["不接真实 provider", "不显示真实价格", "不生成 bookingUrl", "不付款 / 不下单"];
    const subPlans = Array.isArray(detail.subPlans) ? detail.subPlans : [];
    return `<section class="commerce-one-screen-card commerce-global-procurement-plan" aria-label="全球采购计划">
      ...
      <p class="commerce-global-procurement-plan-title"><b>${esc(detail.title || plan.title || "全球采购计划")}</b></p>
      <p>需求摘要：${esc(detail.demandSummary || "等待用户补充采购条件")}</p>
      <p>当前状态：${esc(detail.currentStatusLine || "当前为离线采购规划 / 只整理条件 / 不接真实平台。")}</p>
      ${plan.blockedReason ? `<p>阻断原因：${esc(plan.blockedReason)}</p>` : ""}
      <p>${esc(detail.categoryLine || "采购类型 / 类别：全球采购")}</p>
      <h5>已识别条件</h5>
      ${listHtml(identifiedConditions)}
      <h5>待补充信息</h5>
      ${listHtml(missingInfoList)}
      <h5>安全采购建议</h5>
      ${listHtml(safeGuidanceList)}
      <h5>人工外部搜索入口说明</h5>
      ${listHtml(policyLines)}
      <h5>安全边界说明</h5>
      ${listHtml(safetyBoundaryList)}
      ${subPlans.length ? `<h5>分项计划</h5><div class="commerce-global-procurement-subplans">${subPlans.map(function(subPlan){
        return `<section class="commerce-result-summary-checklist-card">
          <h6>${esc(subPlan.title || "分项")}</h6>
          <div><b>已识别条件</b>${listHtml(subPlan.identifiedConditions || [])}</div>
          <div><b>待补充信息</b>${listHtml(subPlan.missingInfo || [])}</div>
          <div><b>安全建议</b>${listHtml(subPlan.safeGuidance || [])}</div>
          <div><b>当前禁用</b>${listHtml(subPlan.disabled || [])}</div>
        </section>`;
      }).join("")}</div>` : ""}
    </section>`;
  }
  ...
  function commerceIsRestrictedProcurementTask(task){
    ...
    return taskCategory === "generalProcurement"
      && (planCategory === "restricted_or_blocked"
  ...
  function commerceRestrictedProcurementResultPanelHtml(task, options){
    ...
    const advancedDebugHtml = opts.historyMode ? "" : disclosure("查看高级调试信息", `<section class="commerce-simple-flight-advanced-debug" aria-label="高级调试信息">
  ...
  function commerceTicketActivityResultPanelHtml(task, options){
    ...
    return `<section class="commerce-result-summary-panel commerce-one-screen-result commerce-ticket-activity-result" aria-label="门票 / 活动采购计划">
      <div class="commerce-result-summary-head">
        <div class="commerce-result-summary-headline">
          <span>真实结果优先</span>
          <strong>门票 / 活动采购计划</strong>
        </div>
      </div>
      <div class="commerce-one-screen-body">
        <section class="commerce-one-screen-card">
      ...
      <div class="commerce-one-screen-actions" aria-label="门票 / 活动条件操作">
        <button class="cmd-btn gray commerce-external-search-btn" type="button" data-commerce-external-search-kind="web" data-commerce-external-search-url="https%3A%2F%2Fwww.google.com%2Fsearch%3Fq%3D${fields.searchQuery}">打开全网搜索</button>
        <button class="cmd-btn gray commerce-result-summary-copy-btn" type="button" data-commerce-copy-kind="ticketActivity" data-commerce-copy-text="${commerceEncodedCopyText(commerceTicketActivityCopyText(task))}">复制搜索条件</button>
      </div>
      ${commerceReadonlyProviderResultSchemaGateDisclosure(task)}
      ${globalGuardHtml}
      ${globalEvidenceHtml}
  ...
  function commerceProviderResultSourceLabelGateDisplay(task){
    ...
    return gate && typeof gate === "object" ? gate : {
      ...
      audit:{ sourceLabelAuditDraft:{ eventType:"SOURCE_LABEL_GATE_EVALUATION_DRAFT", schemaVersion:"2.1.19", gateState:"closed", blockedReason:"source_label_gate_closed", sourceUrlHost:"none", resultObservedAt:"none", redacted:true } },
  ...
  function commercePriceIntegrityTaxesFeesGateDisplay(task){
    ...
    return gate && typeof gate === "object" ? gate : {
      ...
      audit:{ priceIntegrityAuditDraft:{ eventType:"PRICE_INTEGRITY_EVALUATION_DRAFT", schemaVersion:"2.1.19", gateState:"closed", withheldReason:"price_integrity_gate_closed", providerId:"none", sourceUrlHost:"none", priceObservedAt:"none", taxFeeCompleteness:"none", redacted:true } },
  ...
  function commerceBookingUrlDomainSafetyGateDisplay(task){
    ...
    return gate && typeof gate === "object" ? gate : {
      ...
      audit:{ bookingUrlSafetyAuditDraft:{ eventType:"BOOKING_URL_DOMAIN_SAFETY_EVALUATION_DRAFT", schemaVersion:"2.1.19", gateState:"closed", blockedReason:"booking_url_domain_safety_gate_closed", bookingUrlHost:"none", sourceUrlHost:"none", linkIntent:"none", resultObservedAt:"none", redacted:true } },
  ...
  function commerceBookingUrlDomainSafetyGateDisclosure(task){
    ...
    const body = '<section class="commerce-booking-url-domain-safety-gate-panel" aria-label="bookingUrl domain safety gate">'
      ...
      + '<p>schemaVersion：' + esc(audit.schemaVersion || '2.1.19') + '</p>'
      + '<p>gateState：' + esc(audit.gateState || 'closed') + '</p>'
  ...
  function commerceProviderGateMatrixDashboardDisplay(task){
    ...
    return gate && typeof gate === "object" ? gate : { version:"2.1.19", dashboardStatus:"blocked", mode:"matrix_only", providerActivationState:"no-go", display:{ title:"provider gate matrix dashboard", establishedLine:"provider gate matrix dashboard：dashboard 已建立", statusLine:"status: blocked", modeLine:"mode: matrix only", activationLine:"providerActivationState: no-go", providerConnectionLine:"real provider connection disabled", sandboxLine:"real provider sandbox disabled", networkLine:"real network disabled", priceLine:"real price disabled", bookingUrlLine:"real bookingUrl disabled", orderPaymentLine:"order / payment / checkout disabled", redactedLine:"redacted: true" }, gateMatrix:{ gateMatrixRows:[] }, noGoReasons:{ noGoReasons:[] }, dependencyGraph:{ dependencyGraph:[] }, readinessScore:{ readinessScore:0, readinessMax:100, scoreReason:"real provider activation disabled", scorePolicy:"blocked until all required gates pass", minimumRequiredBeforeActivation:[] }, audit:{ providerGateMatrixAuditDraft:{ eventType:"PROVIDER_GATE_MATRIX_EVALUATION_DRAFT", schemaVersion:"2.1.19", matrixState:"blocked", providerActivationState:"no-go", blockedReason:"provider_gate_matrix_no_go", requiredGateCount:0, passedGateCount:0, failedGateCount:0, reviewedAt:"none", redacted:true } } };
  }
  ...
  function commerceLocalSafetyEvidenceConsoleDisclosure(task){
    ...
    const body = '<section class="commerce-local-safety-evidence-console-panel" aria-label="local safety evidence console">'
      ...
      + '<p>schemaVersion：' + esc(audit.schemaVersion || '2.1.19') + '</p>'
      + '<p>appVersion：' + esc(audit.appVersion || '2.1.19') + '</p>'
  ...
  function commerceResultSummaryHomePanel(completionWorkspace, task, options){
    ...
    if (commerceIsRestrictedProcurementTask(task)) return commerceRestrictedProcurementResultPanelHtml(task, options);
    if (commerceIsTicketActivityTask(task)) return commerceTicketActivityResultPanelHtml(task, options);
    if (!task) return "";
  ...
  function commerceHistoryResultSummaryHomePanel(completionWorkspace, task){
    if (commerceIsSimpleFlightTask(task)) return commerceSimpleFlightResultPanelHtml(task);
    if (commerceIsRestrictedProcurementTask(task)) return commerceRestrictedProcurementResultPanelHtml(task, { historyMode:true });
    if (commerceIsTicketActivityTask(task)) return commerceTicketActivityResultPanelHtml(task, { historyMode:true });
    if (!task) return "";
