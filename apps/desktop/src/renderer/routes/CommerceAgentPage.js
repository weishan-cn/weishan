(function(){
  ...
  function ensureSearchLoaded(host){
    ...
    if (!window.WeishanProviderFailureRetryResilience && !document.querySelector('script[data-weishan-dynamic="WeishanProviderFailureRetryResilience"]')) {
      const resilience = document.createElement("script");
      resilience.src = "./renderer/core/providerFailureRetryResilience.js?v=4.2.8";
      resilience.dataset.weishanDynamic = "WeishanProviderFailureRetryResilience";
      resilience.onload = () => ensureSearchLoaded(host);
  ...
  function currentRealPriceItems(task){
    ...
    return items.filter((item) => item
      ...
      && window.WeishanMerchantNativeSourceEligibilityRouter.isRegisteredSourceId(item.sourceType)
  ...
  function commerceAiBrainDecisionForTask(task){
    ...
    const globalCategory = task && task.globalProcurementIntent && task.globalProcurementIntent.category || "";
    const category = taskCategory === "ecommerce" ? "product" : globalCategory || taskCategory;
  ...
  function commerceShouldShowClarification(task){
    const brain = commerceAiBrainDecisionForTask(task);
    const brainMissing = Array.isArray(brain.missingFields) ? brain.missingFields : [];
    const taskMissing = task && Array.isArray(task.missingFields) ? task.missingFields : [];
    if (brain.procurementCategory === "product") {
      return brainMissing.includes("型号") || taskMissing.includes("型号");
    }
    return brain.intentStatus === "needs_clarification" || taskMissing.length > 0;
  }
  ...
  function commerceResultSummaryPanelHtml(task){
    ...
    if (task && task.category === "ecommerce" && commerceShouldShowClarification(task)) return commerceClarificationResultPanelHtml(task);
  ...
  function detailHtml(task){
    ...
    const readOnlySearchResults = readOnlySearchResultsHtml(task);
    const hasReadOnlySearchResults = !!readOnlySearchResults;
    ...
    const detailSafetyDetails = !blocked ? disclosure("查看安全边界", `
      ...
      <ul class="commerce-safety-list">
        <li>当前不会自动访问真实平台或自动提交表单</li>
        <li>没有真实价格时不会伪造最低价</li>
        <li>用户可以手动点击去平台查看实时价格</li>
        <li>当前不会付款或下单</li>
    ...
    const detailGridHtml = oneScreenResultMode ? "" : `<div class="commerce-detail-grid">
        ...
        ${section("候选方案", hasReadOnlySearchResults ? readOnlySearchResults : candidatesHtml(task), "commerce-section-wide")}
        ...
        ${section("执行边界", `<div class="commerce-risk">当前只做 AI/本地规则辅助搜索、分析、比价、推荐和跳转；不收款、不代下单、不保存账号密码。</div>`)}
        ${section("下一步建议", list(["先查看 2-3 条推荐入口。", "如需更多候选，可展开查看更多候选。", "最终下单或付款前请在外部平台再次确认价格与规则。"]))}
      </div>`;
  ...
  function searchStatusHtml(task, settings, hasProvider){
    ...
    return `<div class="commerce-search-panel">
      ...
      ${missingFields.length ? `<p class="commerce-warning">请补充关键信息：${esc(missingFields.join("、"))}，否则不搜索价格。</p>` : ""}
  ...
  function readOnlySearchResultsHtml(task){
    const summary = task && task.readOnlySearchResultSummary || {};
    ...
    const resultCard = (item) => `<article class="commerce-candidate-card commerce-readonly-search-card">
      <div class="commerce-candidate-head">
        <div>
          <b>${esc(item.title || "只读候选结果")}</b>
          <span>${esc(item.platformName || "")}${item.isOfficial ? " · 官网/官方入口" : ""}${item.trustLevel ? " · " + esc(item.trustLevel === "high" ? "高可信度" : (item.trustLevel === "medium" ? "中等可信度" : "需要复核")) : ""}</span>
        </div>
      ...
      ${item.sourceAttributionName && item.sourceAttributionUrl ? `<p class="commerce-muted">数据提供：${esc(item.sourceAttributionName)} · <button class="cmd-btn gray commerce-source-attribution-link" type="button" data-url="${esc(item.sourceAttributionUrl)}" data-handoff-source="${esc(item.sourceType === "prijsprofeet_public_api" ? "prijsprofeet_attribution" : (item.sourceType || ""))}">查看数据来源</button></p>` : ""}
      ...
      ${item.targetUrl ? `<button class="cmd-btn gray commerce-booking-link" type="button" data-url="${esc(item.targetUrl)}" data-handoff-source="${esc(item.sourceType || "")}">去零售商核验</button>` : `<p class="commerce-warning">目标平台链接缺失，暂不可跳转。</p>`}
    </article>`;
    ...
    return `<div class="commerce-candidates commerce-readonly-search-results" data-commerce-readonly-search-results="true">
      ...
      <details class="commerce-disclosure commerce-platform-template-disclosure">
        ...
        <div class="commerce-disclosure-template" data-commerce-disclosure-html="${esc(encodeURIComponent(`<div class="commerce-candidates">${remaining.map(resultCard).join("") || "<p>暂无更多候选。</p>"}</div>`))}" hidden></div>
        <div class="commerce-disclosure-body" hidden></div>
      </details>
      <div class="commerce-risk">Weishan 不收款、不代下单、不保存账号密码，最终价格与规则以平台页面为准。</div>
    </div>`;
  }
  ...
  function bind(host, tasks, selected){
    ...
    const commerceDelegatedClickHandler = (event) => {
      ...
      if (globalShoppingPublicBetaButton && host.contains(globalShoppingPublicBetaButton)) {
        ...
        output.innerHTML = '<p>全球购 Public Beta</p><p>全球购 Public Beta 已准备</p><p>Public Beta User Journey</p><p>Safe Search Intent Matrix</p><p>User Boundary Panel</p><p>Category Result Simulator</p><p>Read-Only Comparison Board</p><p>Result Trust Badge</p><p>Public Beta Operator Console</p><p>Category Expansion Shell</p><p>Final Offline Beta Audit</p><p>Public Beta Acceptance Board</p><p>Manual Review Required</p><p>Public Beta Visual QA Console</p><p>Trial Scenario Checklist</p><p>No-Transaction Regression Guard</p><p>Visual Acceptance</p><p>Scenario Coverage</p><p>Transaction Boundary</p><p>Flight / Hotel / Product / Restricted 场景已覆盖</p><p>交易按钮保持关闭</p><p>未生成 booking / payment / order / checkout URL</p><p>未打开外部平台</p><p>仍需人工视觉验收</p><p>Public Beta User Onboarding</p><p>Visual Trial Guide</p><p>Safe Feedback Draft</p><p>Readonly Capabilities</p><p>Locked Capabilities</p><p>Privacy Boundary</p><p>你可以查看候选价、费用归一化和官方价锚点</p><p>当前不会付款、下单或出票</p><p>不会保存账号、证件或支付信息</p><p>反馈入口目前仅为草稿，不发送、不上传、不保存用户原文</p><p>feedbackEnabled:false</p><p>uploadEnabled:false</p><p>emailEnabled:false</p><p>externalFormUrl:null</p><p>Public Beta RC Console</p><p>Offline Trial Release Gate</p><p>No Release Mutation</p><p>No Transaction</p><p>No Provider</p><p>No External Open</p><p>Manual Review Required</p><p>Public Beta Stability Audit</p><p>Manual Launch Handoff Pack</p><p>Locked Capabilities</p><p>Next Decision Options</p><p>Continue Testing</p><p>当前仍为只读 Public Beta 候选</p><p>不自动发布、不接 provider、不启用交易</p><p>可继续人工试用和问题记录</p><p>既有 secret scan WARN 仅作为已知警告展示</p><p>Public Beta Manual QA Report Center</p><p>Trial Feedback Safety Gate</p><p>RC Evidence Snapshot</p><p>QA Evidence</p><p>Feedback Safety</p><p>No-Transaction Evidence</p><p>Manual Review Required</p><p>反馈仍为草稿，不发送、不上传、不保存用户原文</p><p>RC 证据快照不写文件、不导出</p><p>人工 QA 后再决定下一阶段</p><p>Public Beta Trial Operations Console</p><p>Manual QA Scenario Runner</p><p>Offline Feedback Review Board</p><p>Scenario Coverage</p><p>Feedback Review</p><p>Next Manual Action</p><p>Manual Review Required</p><p>Flight / Hotel / Product / Restricted / Feedback / No-Transaction / No-Provider 场景已覆盖</p><p>反馈仍保持关闭，不发送、不上传、不保存用户原文</p><p>下一步只能人工复核或继续测试</p><p>不自动发布、不接 provider、不启用交易</p><p>Public Beta Freeze Evidence Summary</p><p>Manual Trial Issue Review Board</p><p>Offline Acceptance Snapshot</p><p>Freeze Evidence</p><p>Issue Review</p><p>Acceptance Snapshot</p><p>Manual Review Required</p><p>冻结证据仅为只读摘要，不修改配置</p><p>问题复核仅为离线视图，不创建真实 issue</p><p>验收快照不写文件、不导出</p><p>仍需人工复核后再决定下一阶段</p><p>Public Beta Acceptance Review Console</p><p>Offline Trial Closure Board</p><p>No-Launch Assurance Gate</p><p>Acceptance Review</p><p>Trial Closure</p><p>No Launch</p><p>当前不发布、不创建 release、不 push</p><p>试用闭环仅为离线视图，不关闭真实任务</p><p>验收复核后仍需人工决定下一阶段</p><p>仍不允许启用 provider、付款、下单或发布</p><p>Public Beta Manual Acceptance Checklist</p><p>Offline User Scenario Pack</p><p>No-Data-Retention Guard</p><p>Acceptance Review</p><p>离线验收清单只用于人工复核，不保存验收记录</p><p>离线场景包仅为样例，不收集真实输入</p><p>无数据留存保护门确认不保存反馈、用户原文、场景输入、验收记录或证据文件</p><p>Public Beta Offline Acceptance Evidence Center</p><p>Manual Scenario Review Board</p><p>Zero-Persistence Regression Gate</p><p>Offline Acceptance Evidence</p><p>Scenario Review</p><p>Zero Persistence</p><p>Manual Review Required</p><p>离线验收证据中心仅为只读展示，不生成证据文件</p><p>人工场景复核板仅为样例复核，不保存场景输入或复核结果</p><p>零持久化回归门确认不保存反馈、用户原文、场景输入、验收记录或证据文件</p><p>provider、联网、外部打开、付款、下单、出票、release、push、launch、反馈提交、上传、issue/task 创建仍保持关闭</p><p>safeToProceedWithManualOfflineAcceptanceReview:false</p><p>Public Beta Final Acceptance Lock</p><p>Offline Release Candidate Audit</p><p>Zero-Action Safety Console</p><p>Final Acceptance</p><p>Release Candidate Audit</p><p>Zero Action Safety</p><p>Manual Review Required</p><p>最终人工验收锁定仅为只读展示，不保存验收记录</p><p>离线 RC 审计不创建 release、不生成审计文件</p><p>零动作安全控制台确认没有任何真实动作执行入口</p><p>provider、联网、外部打开、付款、下单、出票、release、push、launch、反馈提交、上传、issue/task 创建仍保持关闭</p><p>safeToProceedWithManualFinalAcceptanceReview:false</p><p>Public Beta Closure Evidence Archive</p><p>Manual Trial Exit Criteria</p><p>Offline Next-Step Planning Board</p><p>Closure Evidence</p><p>Exit Criteria</p><p>Next-Step Planning</p><p>Manual Review Required</p><p>闭环证据仅为只读归档视图，不写文件</p><p>退出标准不自动通过，不创建 release</p><p>下一步只能继续测试、优化文案、扩展离线场景、人工复核或阻断</p><p>仍不允许启用 provider、付款、下单或发布</p><p>Public Beta Final Readiness Command Center</p><p>Offline Launch Blocker Matrix</p><p>Manual Next-Phase Dossier</p><p>Final Readiness</p><p>Launch Blockers</p><p>Next-Phase Dossier</p><p>Manual Review Required</p><p>当前仍是只读 Public Beta 候选</p><p>发布、provider、联网、付款、下单、出票全部保持阻断</p><p>下一阶段只能继续测试、优化文案、扩展离线场景、人工复核或阻断</p><p>不创建 release、不 push、不启用交易</p><p>Public Beta Candidate Lock</p><p>Final Trial Handoff Console</p><p>No-Provider Production Boundary</p><p>Candidate Scope</p><p>Trial Handoff</p><p>Production Boundary</p><p>Manual Review Required</p><p>当前锁定的是只读 Public Beta 候选范围，不修改配置</p><p>最终试用交接仅为只读摘要，不生成文件</p><p>当前不是 production provider 版本</p><p>provider、联网、外部打开、付款、下单、出票、release、push 全部保持关闭</p><p>Public Beta Candidate Evidence Review</p><p>Trial Operator Notes Panel</p><p>Offline Safety Delta Board</p><p>Candidate Evidence</p><p>Operator Notes</p><p>Safety Delta</p><p>Manual Review Required</p><p>候选证据仅为只读复核，不写文件</p><p>运营备注不保存、不上传、不创建任务</p><p>安全边界未扩大</p><p>provider、联网、外部打开、付款、下单、出票、release、push、launch 仍保持关闭</p><p>Public Beta Candidate QA Freeze</p><p>Trial Feedback Intake Mock</p><p>Offline Regression Evidence Board</p><p>QA Freeze</p><p>Feedback Intake</p><p>Regression Evidence</p><p>Manual Review Required</p><p>QA 冻结仅为只读范围，不修改配置</p><p>反馈入口仅为 Mock，不保存、不上传、不创建任务</p><p>回归证据仅为只读展示，不生成文件</p><p>provider、联网、外部打开、付款、下单、出票、release、push、launch、反馈提交、上传、issue/task 创建仍保持关闭</p><p>feedbackSubmitEnabled:false</p><p>uploadEnabled:false</p><p>issueCreateEnabled:false</p><p>taskCreateEnabled:false</p><p>externalUrl:null</p><p>platformUrl:null</p><p>providerUrl:null</p><p>buyButtonEnabled:false</p><p>checkoutButtonEnabled:false</p><p>paymentButtonEnabled:false</p><p>Flight / Hotel / Product 只读外壳已准备</p><p>Provider-Zero 状态通过</p><p>候选价证据通过</p><p>费用归一化通过</p><p>官方价锚点通过</p><p>安全文案通过</p><p>最终离线审计通过</p><p>仍需人工复核后再决定是否进入下一阶段</p><p>Flight 候选结果</p><p>Hotel 候选结果</p><p>Product 候选结果</p><p>只读对比</p><p>费用已归一化</p><p>已与官方价锚点对比</p><p>Provider-Zero</p><p>需要人工复核</p><p>不代表真实最低价</p><p>不代表最终成交价</p><p>只读搜索计划</p><p>候选价整理</p><p>费用归一化步骤</p><p>官方价锚点步骤</p><p>用户边界确认</p><p>只读候选价</p><p>候选价证据</p><p>来源与时间</p><p>可信度</p><p>风险说明</p><p>官方价锚点</p><p>与官方价对比</p><p>费用归一化</p><p>含税/不含税</p><p>含运费/不含运费</p><p>服务费说明</p><p>归一化价格仅用于辅助比较</p><p>以平台实时页面为准</p><p>当前仍为只读候选证据</p><p>当前不提供付款、下单或出票能力</p><p>不保存账号、证件或支付信息</p><p>用户需在对应平台自行完成下单</p><p>bookingUrl:null</p><p>paymentUrl:null</p><p>orderUrl:null</p><p>payment:false</p><p>order:false</p><p>release:false</p><p>push:false</p><p>download:false</p><p>fileWrite:false</p>';
      ...
      if (globalShoppingProviderZeroLockButton && host.contains(globalShoppingProviderZeroLockButton)) {
        ...
        output.innerHTML = '<p>Provider-Zero Status Panel</p><p>Provider-Zero Status Panel 已准备</p><p>Provider-Zero：未接入真实供应商</p><p>未读取密钥</p><p>未联网调用</p><p>未生成订单</p><p>bookingUrl:null</p><p>payment:false</p><p>order:false</p><p>download:false</p><p>fileWrite:false</p>';
      ...
      if (globalShoppingUserTrustLaunchButton && host.contains(globalShoppingUserTrustLaunchButton)) {
        ...
        output.innerHTML = '<p>User Trust Launch Board</p><p>User Trust Launch Board 已准备</p><p>User Trust Launch 不执行真实 launch</p><p>请在对应平台自行确认价格并完成下单</p><p>Weishan 可尽量带入搜索条件，但用户需在对应平台自行确认价格、登录、填写资料并完成下单</p><p>bookingUrl:null</p><p>payment:false</p><p>order:false</p><p>download:false</p><p>fileWrite:false</p>';
      ...
      if (globalShoppingSafetyCopyCenterButton && host.contains(globalShoppingSafetyCopyCenterButton)) {
        ...
        output.innerHTML = '<p>Public Beta Safety Copy Center</p><p>Public Beta Safety Copy Center 已准备</p><p>Safety Copy 不承诺最低价、最终价或官方背书</p><p>平台实时页面为准</p><p>当前不提供付款、下单或出票能力</p><p>bookingUrl:null</p><p>payment:false</p><p>order:false</p><p>download:false</p><p>fileWrite:false</p>';
