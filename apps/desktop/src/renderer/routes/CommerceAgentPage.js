(function(){
  ...
  function commerceFlightWorkflowPanelHtml(task){
    ...
    const releaseReadiness = typeof releaseDashboardApi.buildFlightWorkflowReleaseReadinessDashboard === "function" ? releaseDashboardApi.buildFlightWorkflowReleaseReadinessDashboard({ releaseVersion:"2.1.70", scenarioSimulationSuite:scenarioSimulation, matrixSummary:safetyTestMatrix, safetyRegressionSummary:safetyRegressionSummary, auditReviewSummary:auditReview, humanReviewChecklistSummary:humanReviewChecklist, finalSafeHandoffPacketSummary:finalSafeHandoffPacket, safeSessionExportPreview:safeExportPreview, operatorConsoleSummary:operatorConsole }) : null;
    ...
    function releaseReadinessHtml(){
      ...
      return '<section class="commerce-flight-release-readiness" data-commerce-flight-release-readiness="true"><h5>机票工作流发布就绪总览</h5><p>发布状态：' + esc(betaLabel) + '</p><p>安全红线</p><p>安全矩阵</p><p>用户复核摘要</p><p>仍被禁止的能力</p><p>安全文案已统一</p><p>当前仍是只读候选证据流程</p><p>不代表真实票价、库存或可出票</p><p>唯珊不会付款、不会下单、不会出票</p><p>唯珊不会上传证件、银行卡或登录凭据</p><p>bookingUrl:null</p><p>payment:false</p><p>order:false</p><button type="button" class="cmd-btn gray" data-commerce-flight-release-readiness-show="true">查看发布就绪总览</button><div data-commerce-flight-release-readiness-output="true"><p>机票工作流发布就绪总览</p><p>发布状态</p><p>安全红线</p><p>安全矩阵</p><p>用户复核摘要</p><p>仍被禁止的能力</p><p>' + esc(betaLabel) + '</p><p>当前仍是只读候选证据流程</p><p>唯珊不会付款、不会下单、不会出票</p></div></section>';
    }
