(function(){
  ...
  function commerceReadOnlyQuoteSessionDisclosure(task){
    ...
    const summary = sample && sample.dryRun && sample.dryRun.sessionSummary || { sessionId:'deterministic-read-only-quote-session-v2.1.55', status:'not_run', redacted:true };
    const body = '<section class="commerce-read-only-quote-session-panel"><h4>Read-Only Quote Session</h4><p>当前只读报价会话</p><p>Read-Only Quote Session</p><p>Session Timeline</p><p>Audit Export</p><p>Session Recovery</p><p>Redacted JSON Preview</p><p>导出脱敏审计预览</p><p>恢复最近一次只读会话</p><p>sessionId: ' + esc(summary.sessionId || 'deterministic-read-only-quote-session-v2.1.55') + '</p><p>status: ' + esc(summary.status || 'not_run') + '</p><p>本导出仅为只读候选证据</p><p>不代表真实最终价、已锁价或可出票</p><p>不包含原始响应、密钥、交易链接或身份信息</p><p>productionProviderEnabled: false</p><p>networkAllowed: false</p><p>autoOpen: false</p><p>autoRefresh: false</p><p>redacted: true</p></section>';
    ...
    const commerceDelegatedClickHandler = (event) => {
      ...
      if ((sandboxImportPreviewButton || sandboxImportConfirmButton || sandboxDryRunButton || sandboxImportClearButton) && host.contains(sandboxImportPreviewButton || sandboxImportConfirmButton || sandboxDryRunButton || sandboxImportClearButton)) {
        ...
        function previewHtml(preview, status, ranking, selectedId, meta){
          ...
          const sessionHtml = metaSafe && metaSafe.sessionSummary ? '<section data-commerce-read-only-session-output="true"><h5>当前只读报价会话</h5><p>Read-Only Quote Session</p><p>Session Timeline</p><p>Audit Export</p><p>Session Recovery</p><p>sessionId: ' + esc(metaSafe.sessionId || metaSafe.sessionSummary.sessionId || '') + '</p><p>status: ' + esc(metaSafe.sessionStatus || metaSafe.sessionSummary.status || 'updated') + '</p><p>本导出仅为只读候选证据</p><p>不代表真实最终价、已锁价或可出票</p><p>不包含原始响应、密钥、交易链接或身份信息</p><button type="button" class="cmd-btn gray" data-commerce-read-only-audit-export-preview="true">导出脱敏审计预览</button> <button type="button" class="cmd-btn gray" data-commerce-replay-last-read-only-run="true" data-commerce-recover-read-only-quote-session="true">恢复最近一次只读会话</button></section>' : '';
      ...
      if (auditExportButton && host.contains(auditExportButton)) {
        ...
        if (old) old.remove();
        card.insertAdjacentHTML("beforeend", '<section data-commerce-read-only-audit-export-output="true"><h5>Audit Export</h5><p>Redacted JSON Preview</p><p>导出脱敏审计预览</p><p>本导出仅为只读候选证据</p><p>不代表真实最终价、已锁价或可出票</p><p>不包含原始响应、密钥、交易链接或身份信息</p><pre>' + esc(JSON.stringify({ exportName: preview && preview.exportModel && preview.exportModel.exportName || 'read_only_quote_audit_export_v1', exportType: 'redacted_json_preview', appVersion: '2.1.55', generatedAt: null, redacted: true, safety: 'redacted' }, null, 2)) + '</pre></section>');
        showCommercePlatformTemplateFeedback("导出脱敏审计预览已生成，不写文件、不上传", false);
        return;
      }
